...
// WatchTrackedFiles runs a watcher in the capsule that prints a line when a
// tracked file changes: inotify when the image has inotifywait, else a
// one-second stat of the files, inside the capsule so no round trip is
// spent while nothing happens. It returns when the capsule ends.
func (d *Docker) WatchTrackedFiles(ctx context.Context, runtime domain.CapsuleRuntime, paths []string, changed func()) error {
	if runtime.Driver != "docker" || runtime.ContainerID == "" || runtime.Status == "stopped" {
		return errors.New("composition has no live Docker capsule")
	}
	var quoted []string
...
if command -v inotifywait >/dev/null 2>&1; then
  dirs="$(for p in $paths; do dirname "$p"; done | sort -u | tr '\n' ' ')"
  inotifywait -m -q -e close_write -e moved_to -e create -e delete -e attrib --format '%w%f' $dirs 2>/dev/null | while IFS= read -r f; do
    for p in $paths; do [ "$f" = "$p" ] && echo "SPIN_CHANGE $p"; done
  done
else
  prev=""
  while :; do
    cur="$(for p in $paths; do stat -c '%n %Y %s %i' "$p" 2>/dev/null || echo "$p -"; done)"
    if [ "$cur" != "$prev" ]; then [ -n "$prev" ] && echo "SPIN_CHANGE *"; prev="$cur"; fi
    sleep 1
  done
fi`
	script = strings.ReplaceAll(script, "'"+paths[0]+"'", "'"+paths[0]+"'")
	cmd := exec.CommandContext(ctx, d.binary, "exec", runtime.ContainerID, "sh", "-c", script)
	stdout, err := cmd.StdoutPipe()
	if err != nil {
		return err
	...
	scanner := bufio.NewScanner(stdout)
	for scanner.Scan() {
		if strings.HasPrefix(scanner.Text(), "SPIN_CHANGE ") {
			changed()
		}
	}
	err = cmd.Wait()
