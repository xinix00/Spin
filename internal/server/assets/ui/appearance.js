...
(() => {
  ...
  function apply(value, save = false) {
    ...
    if (save) {
      let message = 'Je voorkeur is bewaard in deze browser.';
      try { localStorage.setItem(key, JSON.stringify(current)); } catch (_) { message = 'Je voorkeur geldt voor deze pagina; deze browser kan hem niet bewaren.'; }
      const status = document.getElementById('appearance-status');
      if (status) status.textContent = message;
    }
    sync();
    window.dispatchEvent(new CustomEvent('spin-appearance-change', { detail: { ...current } }));
  }
  function sync() {
