...
// A textarea marked data-markdown is edited as markdown: one block per line,
// the prefix decides the style, and the textarea keeps the plain markdown a
// form submit already reads.
function mountMarkdown(root=document){
  if(typeof MarkdownEditor==='undefined')return;
  root.querySelectorAll('textarea[data-markdown]:not([data-markdown-ready])').forEach(area=>{
    area.dataset.markdownReady='1';
    try{MarkdownEditor.mount(area,{placeholder:area.placeholder||''});}catch(error){delete area.dataset.markdownReady;}
  });
}
...
function chatMessage(role,text,key=''){
  const node=document.createElement('article');node.className=`chat-message ${role}`;node.innerHTML=`<div class="chat-role">${role==='user'?`<button type="button" class="chat-edit" title="Bewerk dit bericht en ga vanaf hier verder met een verse agent; de workspace en het gesprek tot hier gaan mee">${icon('edit')}</button>You`:'Agent'}<time class="chat-time" title="${esc(chatEventAt||'')}">${esc(chatClock(chatEventAt))}</time></div><div class="chat-bubble"><div class="md"></div></div>`;const body=node.querySelector('.md');body.dataset.source=text||'';setMarkdown(body,text);chatAppend(node);if(key)chatState.messageNodes.set(key,node);const edit=node.querySelector('.chat-edit');if(edit)edit.onclick=()=>forkFromMessage(node);return node;
}
...
function deliverableChip(item){return `<button class="deliverable-chip" type="button" data-deliverable="${esc(item.id)}"><span class="material-symbols-outlined" aria-hidden="true">${deliverableIcon(item)}</span>${esc(item.name)} · r${item.revision}</button>`;}
...
function workflowDecisionRoutes(question,template){if(question.kind==='action')return `<div class="decision-routes"><span class="decision-route accept">OPNIEUW → ${esc(workflowTargetLabel(template,question.accept_target))}</span><span class="decision-route reject">REJECT → ${esc(workflowTargetLabel(template,question.reject_target))}</span></div>`;return `<div class="decision-routes"><span class="decision-route accept">ACCEPT → ${esc(workflowTargetLabel(template,question.accept_target))}</span><span class="decision-route reject">REJECT → ${esc(workflowTargetLabel(template,question.reject_target))}</span></div>`;}
...
function renderChanges(changes){
  ...
  const list=document.getElementById('changes-list');list.innerHTML=files.length?files.map(file=>`<button type="button" class="change-file${chatState.selectedDiffPath===file.path?' active':''}" data-diff-path="${esc(file.path)}" title="${esc(file.path)}"><span class="material-symbols-outlined tree-icon" aria-hidden="true">${fileIcon(file.path.split('/').pop())}</span><span class="change-status ${changeStatusClass(file.status)}">${esc(String(file.status||'M').trim().charAt(0)||'M')}</span><span class="change-path" title="${esc(file.path)}">${esc(shortPath(file.path))}</span><span class="change-count"><span class="diff-add">+${file.added||0}</span> <span class="diff-del">−${file.deleted||0}</span></span></button>`).join(''):'<div class="empty">Nog geen wijzigingen.</div>';
...
async function renderFullFile(root,file,sessionID,fallback=null){const url=fullFileURL(file,sessionID,fallback);if(!url){root.innerHTML='<div class="diff-empty">Voor dit bestand is geen volledige weergave beschikbaar.</div>';return;}root.innerHTML='<div class="diff-empty">Bestand laden…</div>';try{const loaded=await api(url);if(loaded.binary){root.innerHTML='<div class="diff-empty">Binair bestand.</div>';return;}const marked=addedLines(file);root.innerHTML=`<pre class="code-listing full-file">${String(loaded.content||'').split('\n').map((line,index)=>`<span class="code-line ${marked.has(index+1)?'changed':''}">${highlightCode(line,file.path)||' '}</span>`).join('')}</pre>${loaded.truncated?'<div class="diff-warning">Groot bestand · weergave is afgekapt</div>':''}`;}catch(error){root.innerHTML=`<div class="diff-empty">${esc(error.message||error)}</div>`;}}
...
function closeSessionCapsule(sessionID,fromChat=false){if(!confirm('De capsule van deze stap sluiten? Niet-gecommit werk is al gesynct; de login komt vrij. De chat kan de capsule later weer terughalen.'))return;if(fromChat){chatState.manualClose=true;chatState.socket?.close();clearTimeout(chatState.reconnectTimer);closeDialog('chat-dialog');}return runAction(()=>api(`/api/sessions/${encodeURIComponent(sessionID)}/capsule/stop`,{method:'POST'}),()=>showNotice('Capsule gesloten; de login is vrij.'));}
...
function renderRecording(){
  ...
  const actions=sealing?`<div class="seal-progress"><div class="seal-track ${progressPercent(sealing)===null?'indeterminate':''}"><div class="seal-fill" style="width:${progressPercent(sealing)??100}%"></div></div><small>${esc(sealProgressText(sealing))}</small></div>`:(recording.runtime?.container_id?`<div class="space-before panel-actions"><button class="small-button" data-action="end">End & save</button><button class="danger" data-action="cancel">Cancel</button></div>`:`<div class="space-before panel-actions"><button class="danger" data-action="cancel">Cancel start</button></div>`);
...
function loginsButton(artifact){if(artifact.kind!=='credential')return '';const logins=loginsOf(artifact),held=logins.filter(login=>login.composition_id).length;return `<button class="small-button" data-logins-artifact="${esc(artifact.id)}" title="Logins van deze laag: elke draaiende capsule krijgt er één voor zich alleen">${icon('key')}Logins${logins.length?` · ${logins.length}${held?` (${held} in gebruik)`:''}`:''}</button>`;}
...
function renderComposition(){
  ...
  const loginLine=composition.for_login?`<div class="binding"><span>login</span><span title="Deze capsule kreeg geen login: log in via de shell en bewaar wat je inlogde als nieuwe login">gestart om in te loggen · ${logins.length?logins.join(', ')+' bewaard':`<button class="small-button" data-save-login="${esc(composition.id)}">${icon('key')}Bewaar login</button>`}</span></div>`:logins.length?`<div class="binding"><span>login</span><span title="De login die deze capsule voor zich alleen heeft">${logins.join(', ')}</span></div>`:'';
  ...
  root.innerHTML=`<div class="meta"><span class="tag">${short(composition.id)}</span><span class="tag">${esc(composition.selector)}</span></div>${bindings}${withLayers}${enabled}${mcp}${git}${runtime}${loginLine}${changes}<div class="space-before panel-actions">${ready&&acp?`<button class="small-button" data-action="probe" data-id="${esc(composition.id)}">ACP probe</button>`:''}${ready?`<button class="danger" data-action="stop" data-id="${esc(composition.id)}">Stop</button>`:''}</div>`;
...
function renderJobs(){
  ...
  const cards=jobs.map(job=>{
    ...
    const runHTML=runs.map(run=>{
      ...
      const actions=`${changeAction}${chattable?`<button class="small-button" data-open-acp="${esc(session.id)}" title="${ready?'Chat met de agent van deze stap':'De stap heeft geen capsule nu; de chat start er een (met een vrije login)'}">${icon('chat')}Chat</button>`:''}${closable?`<button class="small-button" data-close-capsule="${esc(session.id)}" title="Sluit de capsule van deze stap; de login die hij vasthoudt komt vrij">${icon('power_settings_new')}Sluit</button>`:''}${retryable?`<button class="small-button" data-retry-session="${esc(session.id)}">${icon('replay')}Retry</button>`:''}`;
...
async function refreshAppPanel(panel){
  ...
  try{
    ...
    body.innerHTML=`${head}${status.services.map(service=>appServiceRowHTML(service,(status.running||[]).find(item=>item.service===service.name))).join('')}<div class="space-before-md panel-actions"><button class="primary" type="button" data-app-start ${status.start==='running'?'disabled':''}>${icon('play_arrow')}${anyRunning?'Herstart':'Start'}</button>${anyRunning?`<button class="danger" type="button" data-app-stop>${icon('stop')}Stop</button>`:''}</div>`;
...
{const zone=document.getElementById('job-dropzone'),input=document.getElementById('job-attachments');zone.onclick=()=>input.click();zone.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();input.click();}};bindDropTarget(zone,addJobAttachmentFiles);}
