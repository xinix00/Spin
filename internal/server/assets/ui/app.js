...
function workflowDecisionRoutes(question,template){if(question.kind==='action')return `<div class="decision-routes"><span class="decision-route accept">OPNIEUW → ${esc(workflowTargetLabel(template,question.accept_target))}</span>${question.reject_target&&question.reject_target!==question.accept_target?`<span class="decision-route reject">OPLOSSEN → ${esc(workflowTargetLabel(template,question.reject_target))}</span>`:''}</div>`;return `<div class="decision-routes"><span class="decision-route accept">ACCEPT → ${esc(workflowTargetLabel(template,question.accept_target))}</span><span class="decision-route reject">REJECT → ${esc(workflowTargetLabel(template,question.reject_target))}</span></div>`;}
...
function renderJobs(){
  ...
  const cards=jobs.map(job=>{
    ...
    const runHTML=runs.map(run=>{
      ...
      const questionButtons=question?.kind==='action'?`<button class="primary" data-accept-question="${esc(question.id)}"${lock}>${icon('replay')}OPNIEUW</button>${question.reject_target&&question.reject_target!==question.accept_target?`<button class="small-button" data-accept-route="${esc(question.id)}"${lock} title="Stuur dit naar ${esc(workflowTargetLabel(template,question.reject_target))} om het te laten oplossen">${icon('alt_route')}OPLOSSEN</button>`:''}`:`${answerButton}${acceptButtons(question,answerButton?'small-button':'primary',lock)}<button class="${answerButton?'small-button':'danger'}" data-reject-question="${esc(question?.id)}"${lock}>${icon('cancel')}REJECT</button>${active&&ready&&acp?`<button class="small-button" data-open-acp="${esc(session.id)}">${icon('chat')}CHAT</button>`:''}`;
