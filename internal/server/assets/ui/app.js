...
function acceptButtons(question,cls,lock){return `<button class="${cls} t-action t-action--success" data-accept-question="${esc(question?.id)}"${lock}>${icon('check')}ACCEPT</button>`;}
...
function renderCodeReviewDecision(revision,annotatable){
  ...
  if(!annotatable||!question){root.innerHTML='';return;}root.dataset.workflowQuestion=question.id;root.innerHTML=`<span>${icon('rate_review')} Review klaar? Comments gaan bij REJECT mee naar de volgende poging.</span><div class="panel-actions t-actions t-actions--end">${acceptButtons(question,'primary','')}<button class="danger t-action t-action--danger" data-reject-question="${esc(question.id)}">${icon('cancel')}REJECT</button></div>`;bindQuestionButtons(root);
}
