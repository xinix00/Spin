import { themeColor, esc, icon, syntaxLanguage, highlightCode, sourceCode, markdown, renderMermaid, setMarkdown } from './render.js';
...
// Shared adapter preserves the textarea as the form value.
function mountMarkdown(root=document){globalThis.TactileMarkdown?.mount(root);}
function openDialog(id){const dialog=document.getElementById(id);if(dialog&&!dialog.open){mountMarkdown(dialog);dialog.showModal();}}
...
function resetChatFeed(){
  ...
  document.getElementById('chat-feed-inner').innerHTML='<div class="chat-welcome t-empty"><strong>ACP Session</strong>De verbinding en agent-context worden opgebouwd…</div>';
}
...
function chatMessage(role,text,key=''){
  const node=document.createElement('article');node.className=`chat-message ${role}`;node.innerHTML=`<div class="chat-role">${role==='user'?`<button type="button" class="chat-edit t-action t-icon-button" title="Bewerk dit bericht en ga vanaf hier verder met een verse agent; de workspace en het gesprek tot hier gaan mee">${icon('edit')}</button>You`:'Agent'}<time class="chat-time" title="${esc(chatEventAt||'')}">${esc(chatClock(chatEventAt))}</time></div><div class="chat-bubble t-layer t-card"><div class="md t-prose"></div></div>`;const body=node.querySelector('.md');body.dataset.source=text||'';setMarkdown(body,text);chatAppend(node);if(key)chatState.messageNodes.set(key,node);const edit=node.querySelector('.chat-edit');if(edit)edit.onclick=()=>forkFromMessage(node);return node;
}
...
function renderACPContent(content,hint='',asSource=false){
  ...
  if(content.type==='text')return asSource?sourceCode(content.text||'',hint,'tool-code syntax-code'):`<div class="md t-prose">${markdown(content.text||'')}</div>`;
  ...
  if(content.type==='resource_link')return `<div class="acp-resource t-card t-card--padded"><strong>${esc(content.name||content.title||'Resource')}</strong><small>${esc(content.uri||'')}</small>${content.description?`<p>${esc(content.description)}</p>`:''}</div>`;
  if(content.type==='resource'&&content.resource){const resource=content.resource;return `<div class="acp-resource t-card t-card--padded"><strong>${esc(resource.uri||'Resource')}</strong>${resource.text!=null?sourceCode(resource.text,resource.uri||hint,'tool-code syntax-code'):`<small>${esc(resource.mimeType||'binary resource')}</small>`}</div>`;}
  return `<pre>${esc(formatToolValue(content))}</pre>`;
}
function appendContent(role,update){
  const content=update.content||{};
  if(content.type!=='text'){
    const node=document.createElement('section');node.className='chat-event content-card t-card';node.innerHTML=`<div class="chat-event-body">${renderACPContent(content)}</div>`;chatAppend(node);renderMermaid(node);return;
  }
...
function appendThought(update){
  ...
  if(!node){node=document.createElement('details');node.className='chat-event chat-thought t-card t-disclosure';node.innerHTML='<summary>Thinking</summary><div class="chat-event-body md t-prose"></div>';chatAppend(node);chatState.thoughtNodes.set(key,node);}
...
function renderToolBody(tool){
  ...
  if(command)sections.push(toolSection('Command',`<pre class="tool-code t-well t-code">${esc(command)}</pre>`));
  else if(tool.rawInput!=null)sections.push(toolSection('Input',`<pre class="tool-code t-well t-code">${esc(formatToolValue(tool.rawInput))}</pre>`));
  (Array.isArray(tool.content)?tool.content:[]).forEach(item=>{
    if(item?.type==='content'){if(item.content?.type==='text')contentTexts.push(String(item.content.text||''));sections.push(toolSection(tool.kind==='read'&&path?`Read · ${shortPath(path,60)}`:'Result',renderACPContent(item.content,path,tool.kind==='read'),tool.kind==='execute'?'tool-output':''));}
      ...
      sections.push(toolSection('Change',`<button class="tool-diff-link t-action" type="button" data-open-change="${esc(changePath)}" title="${esc(changePath)}"><span>${esc(shortPath(changePath||'file',56))}</span><span class="diff-add">+${file.added}</span><span class="diff-del">−${file.deleted}</span></button><div class="tool-change-preview">${renderDiff(file)}</div><div class="tool-diff-note">Klik op het bestand voor de volledige Git-diff.</div>`));
    } else if(item?.type==='terminal')sections.push(toolSection('Terminal',`<pre class="tool-code tool-terminal t-well t-code">${esc(item.terminalId||'attached')}</pre>`));
  });
  if(tool.rawOutput!=null){const result=commandResult(tool.rawOutput);if(!contentTexts.includes(result.text)){const body=tool.kind==='read'&&path&&typeof tool.rawOutput==='string'?sourceCode(result.text,path,'tool-code tool-output syntax-code'):`<pre class="tool-code tool-output t-well t-code">${esc(result.text)}</pre>`;sections.push(toolSection(result.meta?`Output · ${result.meta}`:'Output',body,result.failed?'tool-output error':''));}}
  if(Array.isArray(tool.locations)&&tool.locations.length)sections.push(toolSection('Locations',`<div class="tool-locations">${tool.locations.map(location=>`<button class="t-action" type="button" data-open-change="${esc(workspacePath(location.path))}">${esc(workspacePath(location.path))}${location.line?`:${esc(location.line)}`:''}</button>`).join('')}</div>`));
...
function upsertTool(update){
  ...
  if(!node){node=document.createElement('details');node.className='chat-event tool-card t-card t-disclosure';chatAppend(node);}
  ...
  node.className=`chat-event tool-card t-card t-disclosure ${esc(kind)} ${esc(status)}`;node.innerHTML=`<summary><span class="tool-icon material-symbols-outlined t-icon" title="${esc(kind)}" aria-label="${esc(kind)}">${esc(toolIcon(kind))}</span><span class="tool-summary"><strong class="tool-title">${esc(title)}</strong>${subtitle?`<small class="tool-subtitle">${esc(subtitle)}</small>`:''}</span><span class="tool-state ${esc(status)}" title="${esc(status.replaceAll('_',' '))}"><span class="material-symbols-outlined t-icon" aria-hidden="true">${esc(toolStateIcon(status))}</span>${status==='failed'?'<span class="tool-state-label">mislukt</span>':''}</span><time class="chat-time" title="gestart ${esc(chatClock(startedAt))}${updatedAt&&updatedAt!==startedAt?` · laatste update ${esc(chatClock(updatedAt))}`:''}">${esc(chatClock(updatedAt||startedAt))}</time></summary><div class="tool-body">${renderToolBody(tool)}</div>`;
...
function renderPlan(update){
  ...
  chatState.planNode.innerHTML=`<div class="chat-plan t-card">${(update.entries||[]).map(entry=>`<div class="plan-row"><span>${entry.status==='completed'?icon('check'):entry.status==='in_progress'?icon('pending'):icon('radio_button_unchecked')}</span><span>${esc(entry.content||'')}</span><span class="tag t-supplement t-label t-tag">${esc(entry.status||entry.priority||'pending')}</span></div>`).join('')}</div>`;
}
...
function renderPermission(event){
  ...
  if(event.auto){node.className='chat-event permission-card auto t-card';node.innerHTML=`<div class="chat-event-body"><span class="tool-state completed"><span class="material-symbols-outlined t-icon" aria-hidden="true">check</span></span> Automatisch toegestaan · ${esc(tool.title||'toestemming')}${event.choice?` · ${esc(event.choice)}`:''}</div>`;chatAppend(node);return;}
  node.className='chat-event permission-card t-card';node.innerHTML=`<div class="chat-event-body"><strong>${esc(tool.title||'Permission required')}</strong><div class="permission-actions t-actions">${(params.options||[]).map(option=>`<button class="${String(option.kind||'').startsWith('reject')?'danger t-action--danger':'small-button'} t-action" data-permission-option="${esc(option.optionId)}">${esc(option.name)}</button>`).join('')}</div></div>`;node.querySelectorAll('[data-permission-option]').forEach(button=>button.onclick=()=>{sendChat({type:'permission',request_id:event.request_id,option_id:button.dataset.permissionOption});node.remove();});chatAppend(node);
}
function upsertMeta(key,title,value){
  let node=chatState.metaNodes.get(key);if(!node){node=document.createElement('details');node.className='chat-event acp-meta-card t-card t-disclosure';chatAppend(node);chatState.metaNodes.set(key,node);}
  node.innerHTML=`<summary><span>${esc(title)}</span><span class="tag t-supplement t-label t-tag">ACP</span></summary><div class="chat-event-body"><pre class="tool-code t-well t-code">${esc(formatToolValue(value))}</pre></div>`;
}
...
function connectACPChat(sessionID){
  ...
  if(!chatSessionReady(sessionID)){const session=byID(snapshot.sessions,sessionID),status=document.getElementById('chat-status');status.className='chat-live busy';
...
function openACPChat(sessionID){chatState.restartAllowed=sessionID;
  ...
  // A step that is not running its agent (it waits, or it is done) may
  // have its capsule closed by hand: the login it holds comes free.
  {const run=byID(snapshot.phase_runs,session.phase_run_id),composition=byID(snapshot.compositions,session.prepared_composition_id),button=document.getElementById('chat-close-capsule');button.hidden=!(composition?.runtime&&composition.runtime.status!=='stopped'&&run&&run.status!=='running');button.onclick=()=>closeSessionCapsule(sessionID,true);}
  ...
  const retry=document.getElementById('chat-retry');retry.hidden=!(session.executor!=='action'&&jobWorkedBy(job,currentOperator()));retry.disabled=false;retry.innerHTML=`${icon('replay')}Retry`;retry.dataset.retrySession=sessionID;retry.onclick=()=>retrySession(retry);
...
// Every file in the interface shows what it is: a document, a page, an image,
// a folder. Markdown is a document like any other, with the text-format icon.
function deliverableIcon(item){
  switch(item.kind){
    case 'pdf': return 'picture_as_pdf';
    case 'image': return 'image';
    case 'folder': return item.bundle?.entry?'web':'folder';
    case 'file': return fileIcon(item.bundle?.entry||item.name);
    default: return 'text_format';
  }
}
function deliverableChip(item){return `<button class="deliverable-chip t-supplement t-hover t-label t-action" type="button" data-deliverable="${esc(item.id)}"><span class="material-symbols-outlined t-icon" aria-hidden="true">${deliverableIcon(item)}</span>${esc(item.name)} · r${item.revision}</button>`;}
...
function attachmentHTML(item){return `<a class="attachment-chip t-supplement t-hover t-label t-action" href="/api/job-attachments/${encodeURIComponent(item.id)}" target="_blank" rel="noopener" title="Open ${esc(item.name)} · capsule: ${esc(item.capsule_path)}"><span class="material-symbols-outlined t-icon" aria-hidden="true">${attachmentIcon(item)}</span><span class="attachment-chip-name">${esc(item.name)}</span><span class="attachment-size">${esc(formatBytes(item.size))}</span></a>`;}
...
function renderDeliverableRevision(id){
  ...
  const revisionRoot=document.getElementById('deliverable-revisions');revisionRoot.innerHTML=revisions.map(revision=>`<button type="button" class="revision-button ${revision.id===item.id?'active':''} t-choice t-choice--horizontal" data-deliverable-revision="${esc(revision.id)}">r${revision.revision}${revision.id===latest.id?' · latest':''}</button>`).join('');revisionRoot.querySelectorAll('[data-deliverable-revision]').forEach(button=>button.onclick=()=>renderDeliverableRevision(button.dataset.deliverableRevision));
...
function renderChatDeliverables(sessionID){const session=byID(snapshot.sessions,sessionID),items=latestDeliverables(session?.job_id),attachments=jobAttachments(session?.job_id),root=document.getElementById('chat-deliverables');root.innerHTML=attachments.map(attachmentHTML).join('')+items.map(deliverableChip).join('')||'<span class="hint">Nog geen bijlagen of deliverables.</span>';bindDeliverables(root);}
...
function agentQuestionFormHTML(question){
  ...
  return `<form class="question-form" data-question-form="${esc(question.id)}">${question.items.map(item=>{const name=`${question.id}-${item.id}`,options=(item.options||[]).map(option=>`<label class="question-option t-check-row t-choice t-choice--marked"><input class="t-field" type="radio" name="${esc(name)}" value="${esc(option)}"><span>${esc(option)}</span></label>`).join('');
    return `<fieldset class="question-item t-fieldset" data-question-item="${esc(item.id)}"><legend>${esc(item.question)}</legend>${options}<label class="question-option other t-check-row t-choice t-choice--marked"><input class="t-field" type="radio" name="${esc(name)}" value="__other__" ${options?'':'checked hidden'}><span>${options?'Anders:':''}</span><input class="t-field" type="text" name="${esc(name)}-other" placeholder="${options?'eigen antwoord':'jouw antwoord'}" autocomplete="off"></label></fieldset>`;}).join('')}</form>`;
}
...
function workflowDecisionRoutes(question,template){if(question.kind==='action')return `<div class="decision-routes"><span class="decision-route accept t-supplement t-label t-tag">OPNIEUW → ${esc(workflowTargetLabel(template,question.accept_target))}</span><span class="decision-route reject t-supplement t-label t-tag">REJECT → ${esc(workflowTargetLabel(template,question.reject_target))}</span></div>`;return `<div class="decision-routes"><span class="decision-route accept t-supplement t-label t-tag">ACCEPT → ${esc(workflowTargetLabel(template,question.accept_target))}</span><span class="decision-route reject t-supplement t-label t-tag">REJECT → ${esc(workflowTargetLabel(template,question.reject_target))}</span></div>`;}
...
function acceptButtons(question,cls,lock){return `<button class="${cls} t-action" data-accept-question="${esc(question?.id)}"${lock}>${icon('check')}ACCEPT</button>`;}
...
function renderChanges(changes){
  ...
  const list=document.getElementById('changes-list');list.innerHTML=files.length?files.map(file=>`<button type="button" class="change-file${chatState.selectedDiffPath===file.path?' active':''} t-choice" data-diff-path="${esc(file.path)}" title="${esc(file.path)}"><span class="material-symbols-outlined tree-icon t-icon" aria-hidden="true">${fileIcon(file.path.split('/').pop())}</span><span class="change-status ${changeStatusClass(file.status)}">${esc(String(file.status||'M').trim().charAt(0)||'M')}</span><span class="change-path" title="${esc(file.path)}">${esc(shortPath(file.path))}</span><span class="change-count"><span class="diff-add">+${file.added||0}</span> <span class="diff-del">−${file.deleted||0}</span></span></button>`).join(''):'<div class="empty t-empty">Nog geen wijzigingen.</div>';
...
// The whole file behind a diff: from the commit the diff ends at, or from
// the running capsule for a working tree. Lines the diff added are marked.
const fullFileState={chat:false,job:false};
function fileInRepository(file){const path=workspacePath(file.path);return file.folder&&path.startsWith(file.folder+'/')?path.slice(file.folder.length+1):path;}
// Without a recorded commit (a revision from before, an older runner) the
// Job branch itself is the "after": every file of the Job is on it.
function fullFileURL(file,sessionID,fallback=null){const path=fileInRepository(file);const repository=file.repository||fallback?.repository,ref=file.head||fallback?.ref;if(repository&&ref)return `/api/git/repositories/${encodeURIComponent(repository)}/code/file?${new URLSearchParams({ref,path})}`;if(sessionID)return `/api/sessions/${encodeURIComponent(sessionID)}/file?${new URLSearchParams({path,folder:file.folder||''})}`;return '';}
function jobFileFallback(jobID,filePath){const job=byID(snapshot.jobs,jobID);if(!job)return null;const repositories=job.repositories?.length?job.repositories:[{repository_id:job.git_repository_id,path:''}],path=workspacePath(filePath),match=repositories.find(item=>item.path&&path.startsWith(item.path+'/'))||repositories.find(item=>!item.path)||repositories[0];return {repository:match?.repository_id||job.git_repository_id,ref:job.branch};}
function addedLines(file){const marked=new Set();parseUnifiedPatch(file.patch).forEach(hunk=>hunk.lines.forEach(line=>{if(line.type==='add')marked.add(line.new);}));return marked;}
async function renderFullFile(root,file,sessionID,fallback=null){const url=fullFileURL(file,sessionID,fallback);if(!url){root.innerHTML='<div class="diff-empty t-empty">Voor dit bestand is geen volledige weergave beschikbaar.</div>';return;}root.innerHTML='<div class="diff-empty t-empty">Bestand laden…</div>';try{const loaded=await api(url);if(loaded.binary){root.innerHTML='<div class="diff-empty t-empty">Binair bestand.</div>';return;}const marked=addedLines(file);root.innerHTML=`<pre class="code-listing full-file t-well">${String(loaded.content||'').split('\n').map((line,index)=>`<span class="code-line ${marked.has(index+1)?'changed':''}">${highlightCode(line,file.path)||' '}</span>`).join('')}</pre>${loaded.truncated?'<div class="diff-warning">Groot bestand · weergave is afgekapt</div>':''}`;}catch(error){root.innerHTML=`<div class="diff-empty t-empty">${esc(error.message||error)}</div>`;}}
function renderFullToggle(id,on){const button=document.getElementById(id);if(!button)return;button.classList.toggle('active',on);button.innerHTML=`${icon(on?'difference':'article')}${on?'Diff':'Volledig'}`;}
function renderDiff(file,comments=[]){
  ...
  return `${warnings.map(warning=>`<div class="diff-warning">${esc(warning)}</div>`).join('')}${hunks.length?renderSplitDiff(hunks,language,comments)+renderUnifiedDiff(hunks,language,comments):'<div class="diff-empty t-empty">Voor dit bestand is geen tekst-diff beschikbaar.</div>'}`;
}
function openDiff(path){
  ...
  document.querySelectorAll('[data-diff-path]').forEach(button=>button.classList.toggle('active',workspacePath(button.dataset.diffPath)===normalized));const drawer=document.getElementById('diff-drawer');drawer.hidden=false;renderFullToggle('diff-full-toggle',fullFileState.chat);
  if(file&&fullFileState.chat){document.getElementById('diff-file-name').textContent=file.path;document.getElementById('diff-file-meta').textContent=`${file.status||'M'} · +${file.added||0} −${file.deleted||0} · volledig bestand, gewijzigde regels gemarkeerd`;renderFullFile(document.getElementById('diff-view'),file,chatState.sessionID);return;}
  document.getElementById('diff-file-name').textContent=shortPath(file?.path||normalized||'Change',72);document.getElementById('diff-file-name').title=file?.path||normalized||'';document.getElementById('diff-file-meta').textContent=file?`${file.status||'M'} · +${file.added||0} −${file.deleted||0} · side by side op brede schermen, inline op smalle`:'Wijziging wordt uit de Git-worktree geladen…';document.getElementById('diff-view').innerHTML=file?renderDiff(file):'<div class="diff-empty t-empty">Deze wijziging is nog niet zichtbaar in Git. De worktree wordt opnieuw gelezen…</div>';
}
...
async function exploreRepository(repositoryID){
  ...
  codeEl('tree').innerHTML='<div class="empty t-empty">Branches ophalen…</div>';codeEl('file').innerHTML='<div class="diff-empty t-empty">Kies links een bestand.</div>';codeEl('name').textContent='Selecteer een bestand';codeEl('meta').textContent='Gelezen uit Git.';
  ...
  catch(error){codeEl('tree').innerHTML=`<div class="empty t-empty">${esc(error.message||error)}</div>`;return;}
...
async function loadCodeTree(){
  codeEl('tree').innerHTML='<div class="empty t-empty">Bestanden laden…</div>';
  ...
  }catch(error){const context=codeEl('context');if(context)context.textContent=error.message||'Bestanden niet beschikbaar';codeEl('tree').innerHTML=`<div class="empty t-empty">${esc(error.message||error)}</div>`;}
}
...
function renderCodeNode(node,depth,prefix){const dirs=[...node.dirs.entries()].sort((a,b)=>a[0].localeCompare(b[0])),files=node.files.sort((a,b)=>a.name.localeCompare(b.name));
  return dirs.map(([name,child])=>{const path=prefix?`${prefix}/${name}`:name,open=codeOpenFolders.has(path)||(depth<1&&!codeOpenFolders.has('!'+path));return `<div class="tree-folder${open?' open':''}" data-folder="${esc(path)}"><button type="button" class="tree-row t-choice t-choice--row" data-toggle-folder="${esc(path)}" title="${esc(path)}"><span class="material-symbols-outlined tree-slot t-icon" aria-hidden="true">chevron_right</span><span class="material-symbols-outlined tree-icon folder t-icon" aria-hidden="true">${open?'folder_open':'folder'}</span><span class="tree-name">${esc(name)}</span></button><div class="tree-children" ${open?'':'hidden'}>${renderCodeNode(child,depth+1,path)}</div></div>`;}).join('')+files.map(file=>`<button type="button" class="tree-row tree-file${codeState.path===file.path?' active':''} t-choice t-choice--row" data-code-path="${esc(file.path)}" title="${esc(file.path)}"><span class="tree-slot"></span><span class="material-symbols-outlined tree-icon t-icon" aria-hidden="true">${fileIcon(file.name)}</span><span class="tree-name">${esc(file.name)}</span>${file.size?`<span class="tree-size">${esc(formatBytes(file.size))}</span>`:''}</button>`).join('');}
function renderCodeTree(){const root=codeEl('tree');root.innerHTML=codeState.entries.length?renderCodeNode(codeTreeModel(codeState.entries),0,''):'<div class="empty t-empty">Geen bestanden in deze versie.</div>';
...
async function openCodeFile(path){
  ...
  try{const file=await api(codeURL('file',{ref:codeState.ref,path}));
    ...
    if(file.binary){view.innerHTML='<div class="diff-empty t-empty">Binair bestand.</div>';return;}
    view.innerHTML=`<pre class="code-listing t-well">${String(file.content||'').split('\n').map(line=>`<span class="code-line">${highlightCode(line,path)||' '}</span>`).join('')}</pre>`;
  }catch(error){meta.textContent=error.message||'Niet beschikbaar';view.innerHTML=`<div class="diff-empty t-empty">${esc(error.message||error)}</div>`;}
}
...
async function openJobChanges(button){
  ...
  document.getElementById('job-changes-added').textContent='+0';document.getElementById('job-changes-deleted').textContent='−0';document.getElementById('job-changes-file-count').textContent='0';document.getElementById('job-changes-branch').textContent='Reviewrevisie vastleggen…';document.getElementById('job-changes-list').innerHTML='<div class="empty t-empty">Changes laden…</div>';document.getElementById('job-change-file-name').textContent='Selecteer een bestand';document.getElementById('job-change-file-meta').textContent='Side by side op brede schermen · inline op smalle schermen';document.getElementById('job-change-diff').innerHTML='<div class="diff-empty t-empty">Git-history en worktree worden vergeleken…</div>';document.getElementById('code-review-revisions').innerHTML='';document.getElementById('code-review-comments').innerHTML='<div class="comment-history-note">Reviewrevisie laden…</div>';document.getElementById('code-review-decision').innerHTML='';cancelCodeReviewComment();openDialog('job-changes-dialog');
  try{const bundle=await api(`/api/jobs/${encodeURIComponent(job.id)}/code-reviews`,{method:'POST',body:JSON.stringify({session_id:sessionID,live})});renderCodeReviewBundle(bundle);}catch(error){document.getElementById('job-changes-branch').textContent='Vergelijking mislukt';document.getElementById('job-changes-list').innerHTML=`<div class="empty t-empty">${esc(error.message||error)}</div>`;document.getElementById('job-change-diff').innerHTML='<div class="diff-empty t-empty">De opgeslagen Git-workspace kon niet worden vergeleken.</div>';document.getElementById('code-review-comments').innerHTML='<div class="comment-history-note">Geen reviewrevisie beschikbaar.</div>';}
}
...
function renderCodeReviewBundle(bundle){
  ...
  const revisions=document.getElementById('code-review-revisions');revisions.innerHTML=history.map((item,index)=>`<button type="button" class="revision-button ${item.id===revision.id?'active':''} t-choice t-choice--horizontal" data-code-review-revision="${esc(item.id)}">${esc(codeReviewRevisionLabel(item,index,bundle.latest_revision_id))}</button>`).join('');revisions.querySelectorAll('[data-code-review-revision]').forEach(button=>button.onclick=()=>loadCodeReviewRevision(button.dataset.codeReviewRevision));
...
function renderCodeReviewDecision(revision,annotatable){
  ...
  if(!annotatable||!question){root.innerHTML='';return;}root.dataset.workflowQuestion=question.id;root.innerHTML=`<span>${icon('rate_review')} Review klaar? Comments gaan bij REJECT mee naar de volgende poging.</span><div class="panel-actions t-actions t-actions--end"><button class="primary t-action" data-accept-question="${esc(question.id)}">${icon('check')}ACCEPT</button><button class="danger t-action t-action--danger" data-reject-question="${esc(question.id)}">${icon('cancel')}REJECT</button></div>`;bindQuestionButtons(root);
}
function renderJobChanges(changes){
  ...
  const list=document.getElementById('job-changes-list');list.innerHTML=files.length?files.map(file=>`<button type="button" class="change-file${workspacePath(file.path)===jobChangesState.selectedPath?' active':''} t-choice" data-job-diff-path="${esc(file.path)}" title="${esc(file.path)}"><span class="material-symbols-outlined tree-icon t-icon" aria-hidden="true">${fileIcon(file.path.split('/').pop())}</span><span class="change-status ${changeStatusClass(file.status)}">${esc(String(file.status||'M').trim().charAt(0)||'M')}</span><span class="change-path" title="${esc(file.path)}">${esc(shortPath(file.path))}</span><span class="change-count"><span class="diff-add">+${file.added||0}</span> <span class="diff-del">−${file.deleted||0}</span></span></button>`).join(''):'<div class="empty t-empty">Deze vergelijking bevat geen codewijzigingen.</div>';
  list.querySelectorAll('[data-job-diff-path]').forEach(button=>button.onclick=()=>openJobChangeDiff(button.dataset.jobDiffPath));if(files.length){const selected=files.find(file=>workspacePath(file.path)===jobChangesState.selectedPath);openJobChangeDiff(selected?.path||files[0].path);}else{document.getElementById('job-change-file-name').textContent='Geen changes';document.getElementById('job-change-file-meta').textContent='Deze stap of Job veranderde geen bestanden.';document.getElementById('job-change-diff').innerHTML='<div class="diff-empty t-empty">Geen codewijzigingen gevonden.</div>';}
}
function openJobChangeDiff(path){
  const normalized=workspacePath(path),file=jobChangesState.changes.files.find(candidate=>workspacePath(candidate.path)===normalized),comments=(jobChangesState.bundle?.comments||[]).filter(comment=>workspacePath(comment.path)===normalized);jobChangesState.selectedPath=normalized;document.querySelectorAll('[data-job-diff-path]').forEach(button=>button.classList.toggle('active',workspacePath(button.dataset.jobDiffPath)===normalized));document.getElementById('job-change-file-name').textContent=file?.path||normalized||'Change';document.getElementById('job-change-file-meta').textContent=file?`${file.status||'M'} · +${file.added||0} −${file.deleted||0} · ${fullFileState.job?'volledig bestand, gewijzigde regels gemarkeerd':'side by side op brede schermen, inline op smalle'}`:'Selecteer een bestand';const root=document.getElementById('job-change-diff');renderFullToggle('job-change-full-toggle',fullFileState.job);if(file&&fullFileState.job){renderFullFile(root,file,'',jobFileFallback(jobChangesState.jobID,file.path));return;}root.innerHTML=file?renderDiff(file,comments):'<div class="diff-empty t-empty">Selecteer links een bestand om de diff te bekijken.</div>';root.onmouseup=jobChangesState.bundle?.annotatable?captureCodeReviewSelection:null;
}
...
function showBanner(text,kind){
  const center=document.getElementById('messages'),item=document.createElement('div');item.className=`message t-notification t-notice ${kind==='notice'?'notice':'error'}`;item.dataset.tone=kind==='notice'?'success':'danger';
  item.innerHTML=`<span class="material-symbols-outlined t-icon" aria-hidden="true">${kind==='notice'?'check_circle':'error'}</span><span class="message-text t-notification-text">${esc(String(text||''))}</span><button class="message-close t-dismiss t-action t-icon-button" type="button" aria-label="Sluiten"><span class="material-symbols-outlined t-icon" aria-hidden="true">close</span></button>`;
...
const terminalTheme = {
  ...
  selectionBackground: themeColor('--terminal-selection'),
  black: themeColor('--terminal-black'),
  brightBlack: themeColor('--terminal-bright-black'),
  green: themeColor('--terminal-green'),
  brightGreen: themeColor('--terminal-bright-green'),
  blue: themeColor('--terminal-blue'),
  brightBlue: themeColor('--terminal-bright-blue'),
  yellow: themeColor('--terminal-yellow'),
...
function useLayers(selector,withSelectors=[],forLogin=false,forLoginPrivate=false){return runAction(()=>api('/api/use',{method:'POST',body:JSON.stringify({operator:currentOperator(),selector,with_selectors:withSelectors,profile:'default',for_login:forLogin,for_login_private:forLoginPrivate})}),composition=>{if(composition.warnings?.length)showError(new Error(composition.warnings.join('; ')));if(forLogin)showNotice('Capsule gestart zonder login: log in via de shell en druk daarna op Bewaar login.');});}
async function closeSessionCapsule(sessionID,fromChat=false){if(!await TactileDialog.confirm('De capsule van deze stap sluiten? Niet-gecommit werk is al gesynct; de login komt vrij. De chat kan de capsule later weer terughalen.'))return;if(fromChat){chatState.manualClose=true;chatState.socket?.close();clearTimeout(chatState.reconnectTimer);closeDialog('chat-dialog');}return runAction(()=>api(`/api/sessions/${encodeURIComponent(sessionID)}/capsule/stop`,{method:'POST'}),()=>showNotice('Capsule gesloten; de login is vrij.'));}
function saveLogin(compositionID){return runAction(()=>api(`/api/compositions/${encodeURIComponent(compositionID)}/login`,{method:'POST'}),logins=>showNotice(logins.map(login=>`Login ${login.number} van ${login.key.split('/').slice(1).join('/')} bewaard`).join('; ')+'; de capsule is gesloten en de login is vrij.'));}
function deleteLogin(loginID){return runAction(()=>api(`/api/logins/${encodeURIComponent(loginID)}`,{method:'DELETE'}),()=>{if(document.getElementById('logins-dialog').open)renderLogins();});}
...
function probeACP(id){return runAction(()=>api(`/api/compositions/${encodeURIComponent(id)}/acp/probe`,{method:'POST',body:JSON.stringify({operator:currentOperator()})}),probe=>TactileDialog.notice(`ACP v1 handshake geslaagd · ${probe.enablement?.command||''}\n\n${JSON.stringify(probe.handshake,null,2)}`));}
...
function renderRecording(){
  ...
  if(!recording){
    ...
    if(composition?.for_login){
      ...
      root.innerHTML=`<div class="recorder-idle"><p>Shell van <strong>${esc(composition.selector)}</strong> om in te loggen. ${saved?'Login bewaard; deze capsule sluit zo.':'Log in via de shell en druk daarna op Bewaar login: alleen de bijgehouden bestanden worden bewaard, er komt geen laag bij, en de capsule sluit daarna vanzelf.'}</p>${saved?`<button class="danger t-action t-action--danger" type="button" id="recorder-stop-login">${icon('stop')}Stop</button>`:`<button class="primary t-action" type="button" id="recorder-save-login">${icon('key')}Bewaar login</button>`}</div>`;
    ...
    root.innerHTML=`<div class="recorder-idle"><p>${composition?`Shell van <strong>${esc(composition.selector)}</strong>. Er wordt niets opgenomen.`:'Geen opname. Een nieuwe laag begint met een opname in een verse capsule.'}</p><button class="primary t-action" type="button" id="recorder-new-layer">${icon('fiber_manual_record')}Nieuwe laag</button></div>`;root.querySelector('#recorder-new-layer').onclick=()=>openLayerDialog({scope:'user'});updateTerminalControls();return;}
  ...
  const runtime=recording.runtime?.container_id?'<small>Werk in de shell hieronder; + shell opent een tweede in dezelfde capsule.</small>':`<div class="seal-progress"><div class="seal-track ${progressPercent(starting)===null?'indeterminate':''} t-progress"><div class="seal-fill" style="width:${progressPercent(starting)??100}%"></div></div><small>${esc(startProgressText(starting||{}))}</small></div>`;
  ...
  const actions=sealing?`<div class="seal-progress"><div class="seal-track ${progressPercent(sealing)===null?'indeterminate':''} t-progress"><div class="seal-fill" style="width:${progressPercent(sealing)??100}%"></div></div><small>${esc(sealProgressText(sealing))}</small></div>`:(recording.runtime?.container_id?`<div class="space-before panel-actions t-actions t-actions--end"><button class="small-button t-action" data-action="end">End & save</button><button class="danger t-action t-action--danger" data-action="cancel">Cancel</button></div>`:`<div class="space-before panel-actions t-actions t-actions--end"><button class="danger t-action t-action--danger" data-action="cancel">Cancel start</button></div>`);
...
// A credential layer hands its logins out: one per running capsule. The
// dialog lists them, who holds which, and starts a capsule to log in once
// more.
const loginsState={artifactID:'',openFiles:new Set()};
// The files of a login, by folder, each with a way out: excluding a file
// or a folder adds it to the layer's excludes and takes it out of every
// login at once, so an agent that starts hoarding is stopped in one click.
function toggleLoginFiles(id){if(loginsState.openFiles.has(id))loginsState.openFiles.delete(id);else loginsState.openFiles.add(id);renderLogins();}
async function renderLoginFiles(id){const root=document.querySelector(`[data-login-files-for="${CSS.escape(id)}"]`);if(!root)return;root.hidden=false;root.innerHTML='<span class="hint">Laden…</span>';
  try{const files=await api(`/api/logins/${encodeURIComponent(id)}/files`);if(!files.length){root.innerHTML='<span class="hint">Leeg.</span>';return;}
    const folders=new Map();files.forEach(file=>{const slash=file.path.lastIndexOf('/'),folder=file.path.slice(0,slash+1);if(!folders.has(folder))folders.set(folder,[]);folders.get(folder).push(file);});
    root.innerHTML=[...folders.entries()].map(([folder,items])=>`<div class="login-folder t-layer t-well"><div class="login-folder-head"><code>${esc(folder)}</code><span class="tree-size">${items.length} · ${esc(formatBytes(items.reduce((sum,file)=>sum+file.size,0)))}</span><button class="small-button t-action" data-exclude-path="${esc(folder)}" title="Deze map voortaan niet meer bijhouden en uit alle logins halen">${icon('block')}Map uitsluiten</button></div>${items.map(file=>`<div class="login-file"><span class="tree-name">${esc(file.path.slice(folder.length))}</span><span class="tree-size">${esc(formatBytes(file.size))}</span><button class="icon-button t-action t-icon-button" data-exclude-path="${esc(file.path)}" title="Dit bestand voortaan niet meer bijhouden en uit alle logins halen" aria-label="Uitsluiten">${icon('block')}</button></div>`).join('')}</div>`).join('');
    root.querySelectorAll('[data-exclude-path]').forEach(button=>button.onclick=async()=>{const path=button.dataset.excludePath;if(!await TactileDialog.confirm(`${path} voortaan niet meer bijhouden en uit alle logins van deze laag halen?`))return;button.disabled=true;try{const result=await api(`/api/artifacts/${encodeURIComponent(loginsState.artifactID)}/tracked/exclude`,{method:'POST',body:JSON.stringify({path})});showNotice(`${path} uitgesloten · ${result.removed} bestand${result.removed===1?'':'en'} uit de logins gehaald`);await refresh(true);renderLogins();}catch(error){showError(error);button.disabled=false;}});
  }catch(error){root.innerHTML=`<span class="warning">${esc(error.message||error)}</span>`;}}
function layerKey(artifact){return `${artifact.subject||''}/${artifact.kind}:${artifact.name}`;}
function loginsOf(artifact){const key=layerKey(artifact);return (snapshot.logins||[]).filter(login=>login.key===key);}
function loginHolderLabel(compositionID){const composition=byID(snapshot.compositions,compositionID);if(!composition)return `capsule ${short(compositionID)}`;const session=composition.session_id?byID(snapshot.sessions,composition.session_id):null,job=session?byID(snapshot.jobs,session.job_id):null;if(job)return `Job ${job.title}`;if(composition.for_login)return 'capsule om in te loggen';return `capsule ${short(composition.id)} van ${composition.operator}`;}
function loginsButton(artifact){if(artifact.kind!=='credential')return '';const logins=loginsOf(artifact),held=logins.filter(login=>login.composition_id).length;return `<button class="small-button t-action" data-logins-artifact="${esc(artifact.id)}" title="Logins van deze laag: elke draaiende capsule krijgt er één voor zich alleen">${icon('key')}Logins${logins.length?` · ${logins.length}${held?` (${held} in gebruik)`:''}`:''}</button>`;}
function openLogins(artifactID){loginsState.artifactID=artifactID;renderLogins();openDialog('logins-dialog');}
function renderLogins(){
  const artifact=byID(snapshot.artifacts,loginsState.artifactID);if(!artifact)return;
  const logins=loginsOf(artifact),mayRemove=artifact.subject===currentOperator()||artifact.created_by===currentOperator()||authState.user?.role==='admin',tracks=(artifact.tracked_paths||[]).length>0;
  document.getElementById('logins-new').disabled=!tracks;document.getElementById('logins-new').title=tracks?'Start een capsule op deze laag zonder login; log in via de shell en druk daarna op Bewaar login':'Kies eerst in Inhoud welke bestanden of mappen deze laag bijhoudt';
  document.getElementById('logins-untracked').hidden=tracks;
  ...
  document.getElementById('logins-body').innerHTML=logins.length?logins.map(login=>`<div class="login-row t-layer t-card"><div class="login-head"><span class="material-symbols-outlined login-icon t-icon" aria-hidden="true">key</span><input class="login-name t-field" data-login-name="${esc(login.id)}" value="${esc(login.name||'')}" placeholder="Login ${login.number}" aria-label="Naam van deze login" ${mayRemove?'':'disabled'}><span class="login-tags">${login.disabled?'<span class="tag disabled-tag t-supplement t-label t-tag" title="Uitgezet: deze login gaat naar geen enkele capsule">uit</span>':''}${login.owner?`<span class="tag t-supplement t-label t-tag" title="Alleen voor ${esc(login.owner)}">privé · ${esc(login.owner)}</span>`:artifact.scope==='global'?'<span class="tag t-supplement t-label t-tag">voor iedereen</span>':''}${login.composition_id?`<span class="tag held t-supplement t-label t-tag">in gebruik · ${esc(loginHolderLabel(login.composition_id))}</span>`:'<span class="tag free t-supplement t-label t-tag">vrij</span>'}</span><span class="login-actions t-actions"><button class="small-button t-action" data-login-toggle="${esc(login.id)}" data-login-disabled="${login.disabled?'1':'0'}" title="${login.disabled?'Weer meedoen in de pool':'Uitzetten, bijvoorbeeld als dit account door zijn tokens heen is; een draaiende capsule stapt meteen over op een andere login'}">${icon(login.disabled?'play_arrow':'pause')}${login.disabled?'Aanzetten':'Uitzetten'}</button><button class="small-button t-action" data-login-files="${esc(login.id)}" title="Welke bestanden deze login bevat; wat een agent verzamelt kun je hier uitsluiten">${icon('folder_open')}Bestanden</button>${mayRemove?`<button class="danger t-action t-action--danger" data-delete-login="${esc(login.id)}" title="${login.composition_id?'Verwijder deze login en sluit de capsule die hem vasthoudt':'Verwijder deze login; de capsules die erna starten krijgen een andere'}">${icon('delete')}Verwijder</button>`:''}</span></div><small class="login-meta">${login.files} bestand${login.files===1?'':'en'} · ${esc(formatBytes(login.bytes||0))} · bijgewerkt ${esc(new Date(login.updated_at).toLocaleString())}${login.last_used_at?` · laatst uitgedeeld ${esc(new Date(login.last_used_at).toLocaleString())}`:' · nog niet uitgedeeld'}</small><div class="login-files" data-login-files-for="${esc(login.id)}" hidden></div></div>`).join(''):'<div class="empty t-empty">Nog geen logins.</div>';
  ...
  document.getElementById('logins-body').querySelectorAll('[data-delete-login]').forEach(button=>button.onclick=async()=>{const login=byID(snapshot.logins,button.dataset.deleteLogin);if(await TactileDialog.confirm(login?.composition_id?'Deze login verwijderen? De capsule die hem vasthoudt wordt gesloten.':'Deze login verwijderen?'))deleteLogin(button.dataset.deleteLogin);});
}
// A capsule started to log in lives only for the console: closing the
// console closes the capsule too, so no login stays held by a shell nobody
// looks at. One that saved its login is already closing.
document.getElementById('capsule-dialog').addEventListener('close',()=>{const composition=activeComposition();if(!composition?.for_login||Object.keys(composition.logins||{}).length)return;stopComposition(composition.id);showNotice('Inlogcapsule gesloten zonder een login te bewaren.');});
document.getElementById('logins-open-contents').onclick=()=>{const artifact=byID(snapshot.artifacts,loginsState.artifactID);if(!artifact)return;closeDialog('logins-dialog');openContents(`/api/artifacts/${encodeURIComponent(artifact.id)}/contents`,artifactSelector(artifact),artifact);};
document.getElementById('logins-new').onclick=()=>{const artifact=byID(snapshot.artifacts,loginsState.artifactID);if(!artifact)return;const privateLogin=artifact.scope==='global'&&document.getElementById('logins-private').checked;closeDialog('logins-dialog');setTab('environments');openConsole();useLayers(artifactSelector(artifact),[],true,privateLogin);};
function contentsButton(artifact){const summary=trackedSummary(artifact.tracked_paths||[],artifact.tracked_excludes||[]);return artifact.snapshot?.contents?`<button class="small-button t-action" data-contents-artifact="${esc(artifact.id)}" title="Alles in deze laag en in haar logins; vink bestanden of mappen aan die Spin tussen Sessions bijhoudt${summary?` · ${esc(summary)}`:''}">${icon('inventory_2')}Inhoud${summary?` · ${esc(summary.replace(' bijgehouden',''))}`:''}</button>`:'';}
...
function ruleControls(path,isFolder){const state=ruleState(path),label=isFolder?'map':'bestand';
  switch(state){
    case 'included':return `<span class="tag rule-tag included t-supplement t-label t-tag" title="Staat in de lijst Bijhouden">bijgehouden</span><button class="icon-button rule-remove t-action t-icon-button" type="button" data-rule-remove="${esc(path)}" title="Uit de lijst halen" aria-label="Regel weghalen">${icon('close')}</button>`;
    case 'excluded':return `<span class="tag rule-tag excluded t-supplement t-label t-tag" title="Staat in de lijst Uitgesloten">uitgesloten</span><button class="icon-button rule-remove t-action t-icon-button" type="button" data-rule-remove="${esc(path)}" title="Uitsluiting weghalen" aria-label="Regel weghalen">${icon('close')}</button>`;
    case 'covered':return `<span class="tag rule-tag covered t-supplement t-label t-tag" title="Valt onder bijgehouden map ${esc(includedBy(path))}">via map</span><button class="small-button rule-button t-action" type="button" data-rule-exclude="${esc(path)}" title="Deze ${label} uitsluiten van de bijgehouden map">${icon('block')}Uitsluiten</button>`;
    case 'excluded-by':return `<span class="tag rule-tag excluded t-supplement t-label t-tag" title="Valt onder uitgesloten map ${esc(excludedBy(path))}">via uitgesloten map</span>`;
    default:return `<button class="small-button rule-button t-action" type="button" data-rule-include="${esc(path)}" title="Deze ${label} bijhouden${isFolder?' (alles erin, ook wat er later bij komt)':''}">${icon('add')}Bijhouden</button>`;
  }}
function renderTrackedRules(){const includes=[...contentsState.tracked].sort(),excludes=[...contentsState.excluded].sort();
  return `<div class="tracked-rules"><div class="t-card"><strong>Bijhouden</strong>${includes.length?includes.map(path=>`<div class="rule-row"><code>${esc(path)}</code><span class="tag t-supplement t-label t-tag">${path.endsWith('/')?'map':'bestand'}</span><button class="icon-button rule-remove t-action t-icon-button" type="button" data-rule-remove="${esc(path)}" aria-label="Weghalen" title="Uit de lijst halen">${icon('close')}</button></div>`).join(''):'<div class="hint">Nog niets. Kies hieronder een bestand of map met Bijhouden.</div>'}</div><div class="t-card"><strong>Uitgesloten</strong>${excludes.length?excludes.map(path=>`<div class="rule-row"><code>${esc(path)}</code><span class="tag t-supplement t-label t-tag">${path.endsWith('/')?'map':'bestand'}</span><button class="icon-button rule-remove t-action t-icon-button" type="button" data-rule-remove="${esc(path)}" aria-label="Weghalen" title="Uitsluiting weghalen">${icon('close')}</button></div>`).join(''):'<div class="hint">Niets uitgesloten. Binnen een bijgehouden map kun je bestanden en mappen uitsluiten.</div>'}</div></div>`;}
...
function contentsTreeModel(entries){const root={dirs:new Map(),files:[]};entries.forEach(entry=>{const parts=entry.path.replace(/^\//,'').split('/');let node=root;parts.slice(0,-1).forEach(part=>{if(!node.dirs.has(part))node.dirs.set(part,{dirs:new Map(),files:[]});node=node.dirs.get(part);});node.files.push({name:parts.at(-1),path:entry.path,size:entry.bytes,logins:entry.logins||[],source:entry.source||''});});return root;}
...
function renderContentsNode(node,depth,prefix,track,openAll){
  ...
  return dirs.map(([name,child])=>{const path=prefix?`${prefix}/${name}`:name,open=openAll||contentsOpenFolders.has(path),below=contentsFilesBelow(child),bytes=below.reduce((sum,file)=>sum+(file.size||0),0);
    return `<div class="tree-folder${open?' open':''}"><div class="tree-row t-choice--row"><button type="button" class="tree-row-toggle t-choice" data-toggle-contents-folder="${esc(path)}" title="${esc('/'+path)}"><span class="material-symbols-outlined tree-slot t-icon" aria-hidden="true">chevron_right</span><span class="material-symbols-outlined tree-icon folder t-icon" aria-hidden="true">${open?'folder_open':'folder'}</span><span class="tree-name">${esc(name)}</span></button>${track?`<span class="rule-controls">${ruleControls(folderKey(path),true)}</span>`:''}<span class="tree-size">${below.length} · ${esc(formatBytes(bytes))}</span></div><div class="tree-children" ${open?'':'hidden'}>${open?renderContentsNode(child,depth+1,path,track,openAll):''}</div></div>`;}).join('')+
    files.slice(0,1500).map(file=>`<div class="tree-row tree-file t-choice t-choice--row"><span class="tree-slot"></span><span class="material-symbols-outlined tree-icon t-icon" aria-hidden="true">${fileIcon(file.name)}</span><span class="tree-name" title="${esc(file.path)}">${esc(file.name)}</span>${file.logins?.length?`<span class="tag login-tag t-supplement t-label t-tag" title="${file.source==='login'?'Niet in de laag; kwam erbij in login '+file.logins.join(', '):'Ook in login '+file.logins.join(', ')}">${file.source==='login'?'uit ':''}login ${file.logins.join(', ')}</span>`:''}${track?`<span class="rule-controls">${ruleControls(file.path,false)}</span>`:''}<span class="tree-size">${esc(formatBytes(file.size||0))}</span></div>`).join('')+(files.length>1500?`<div class="tree-row tree-file t-choice t-choice--row"><span class="tree-slot"></span><span class="tree-name hint">nog ${files.length-1500} bestanden in deze map; filter op pad om ze te zien</span></div>`:'');
}
...
function renderContentsList(){
  ...
  root.innerHTML=(track?renderTrackedRules():'')+(rows.length?`${rows.length>1500&&filter?`<p class="hint">${rows.length} bestanden passen; mappen staan dicht, klap open wat je zoekt of verfijn het filter.</p>`:''}<div class="code-tree contents-tree">${renderContentsNode(contentsState.model,0,'',track,openAll)}</div>`:'<div class="empty t-empty">Niets gevonden.</div>');
...
function renderComposition(){
  ...
  if(!composition){root.innerHTML='<div class="empty t-empty">Nog geen draaiende omgeving.</div>';return;}
  ...
  const git=(composition.workspaces?.length?composition.workspaces:composition.git?[composition.git]:[]).map(workspace=>`<div class="binding"><span>GIT${workspace.path?` · ${esc(workspace.path)}`:''}</span><span>${esc(workspace.repository_name)} · ${workspace.mode==='reference'?`ter referentie op ${esc(workspace.base_ref)}`:`${esc(workspace.head_ref)} → ${esc(workspace.target_ref)}`} · ${esc(workspace.credential_scope||'public')}${workspace.login?' · '+esc(workspace.provider+':'+workspace.login):''}</span></div>`).join('');
  ...
  const logins=Object.entries(composition.logins||{}).map(([key,id])=>{const login=byID(snapshot.logins,id);return `${esc(key.split('/').slice(1).join('/'))} → ${login?`Login ${login.number}`:esc(short(id))}`;});
  const loginLine=composition.for_login?`<div class="binding"><span>login</span><span title="Deze capsule kreeg geen login: log in via de shell en bewaar wat je inlogde als nieuwe login">gestart om in te loggen · ${logins.length?logins.join(', ')+' bewaard':`<button class="small-button t-action" data-save-login="${esc(composition.id)}">${icon('key')}Bewaar login</button>`}</span></div>`:logins.length?`<div class="binding"><span>login</span><span title="De login die deze capsule voor zich alleen heeft">${logins.join(', ')}</span></div>`:'';
  const changes=composition.capsule_changes?.files?`<div class="binding"><span>gewijzigd</span><span title="Buiten de workspace, sinds de start van de capsule">${composition.capsule_changes.files} bestand${composition.capsule_changes.files===1?'':'en'} · ${esc(formatBytes(composition.capsule_changes.bytes))} <button class="small-button t-action" id="composition-changes-button">${icon('inventory_2')}Bekijk</button></span></div>`:'';
  const ready=composition.runtime&&composition.runtime.status!=='stopped';
  const acp=(composition.enabled||[]).some(item=>item.name==='acp');
  root.innerHTML=`<div class="meta"><span class="tag t-supplement t-label t-tag">${short(composition.id)}</span><span class="tag t-supplement t-label t-tag">${esc(composition.selector)}</span></div>${bindings}${withLayers}${enabled}${mcp}${git}${runtime}${loginLine}${changes}<div class="space-before panel-actions t-actions t-actions--end">${ready&&acp?`<button class="small-button t-action" data-action="probe" data-id="${esc(composition.id)}">ACP probe</button>`:''}${ready?`<button class="danger t-action t-action--danger" data-action="stop" data-id="${esc(composition.id)}">Stop</button>`:''}</div>`;
  ...
  root.querySelectorAll('[data-save-login]').forEach(button=>button.onclick=()=>{button.disabled=true;saveLogin(button.dataset.saveLogin);});
...
function renderArtifacts(){
  ...
  if(!artifacts.length){root.innerHTML='<div class="empty t-empty">Nog geen lagen. Open de recorder om <code>tool:git</code> als eerste laag te maken.</div>';return;}
  root.innerHTML=artifacts.map(artifact=>{
    const identity=artifact.subject?`${artifact.scope}:${artifact.subject}`:artifact.scope;
    const use=canUse(artifact)?`<button class="small-button t-action" data-action="use" data-selector="${esc(artifactSelector(artifact))}">${icon('arrow_forward')}Start</button>`:'';
    const next=`<button class="small-button t-action" data-record-from="${esc(artifactSelector(artifact))}">${icon('add')}Laag</button><button class="small-button t-action" data-edit-artifact="${esc(artifactSelector(artifact))}" title="Neem deze laag opnieuw op met al haar instellingen en voeg toe wat ontbreekt; End &amp; save vervangt de huidige versie, ook voor lagen die erop gebouwd zijn">${icon('edit')}Bewerk</button>`;
    const remove=artifact.created_by===currentOperator()||authState.user?.role==='admin'?`<button class="danger t-action t-action--danger" data-remove-artifact="${esc(artifact.id)}" data-artifact-label="${esc(artifactSelector(artifact))}">${icon('delete')}Verwijder</button>`:'';
    ...
    const settingsLine=acp&&artifact.id===agentLayer.id&&!(options&&!options.error)?`<div class="agent-settings">${autoAcceptPick}</div>`:acp&&options&&!options.error&&artifact.id===agentLayer.id?`<div class="agent-settings">${autoAcceptPick}${pick('mode','Modus',options.modes,settings.mode,'automatisch · full access als de agent dat kent')}<label class="agent-setting"><span>Model</span><input class="t-field" data-agent-setting="model" data-agent-layer="${esc(agentLayer.id)}" list="agent-model-options-${esc(agentLayer.id)}" value="${esc(settings.model||'')}" placeholder="agent default" autocomplete="off" title="Kies uit de lijst of typ een model-id dat de agent kent; leeg is het standaardmodel van je account"><datalist id="agent-model-options-${esc(agentLayer.id)}">${(options.models||[]).map(item=>`<option value="${esc(item.value)}">${esc(item.name||item.value)}${item.description?` · ${esc(item.description)}`:''}</option>`).join('')}</datalist></label>${pick('reasoning_effort','Reasoning',options.reasoning_efforts,settings.reasoning_effort,'agent default')}</div>`:'';
    return `<article class="artifact t-card t-record t-record--stacked"><div class="t-record-content"><div class="artifact-title"><span>${esc(artifactSelector(artifact))}</span><span class="tag t-supplement t-label t-tag">L${artifactLayer(artifact)}</span>${artifactLineage(artifact).version>1?`<span class="tag capability t-supplement t-label t-tag" title="Nieuwe versie van deze laag; oudere versies blijven bestaan voor lagen die erop gebouwd zijn">v${artifactLineage(artifact).version}</span>`:''}<span class="tag ${esc(artifact.kind)} t-supplement t-label t-tag">${esc(artifact.kind)}</span><span class="tag t-supplement t-label t-tag">${esc(identity)}</span>${artifact.sensitivity==='secret'?'<span class="tag secret t-supplement t-label t-tag">secret</span>':''}</div><small title="${esc(artifact.snapshot_digest)}">${esc(artifact.profile)} · ${esc(artifact.snapshot?.driver||'legacy')} · ${esc(String(artifact.snapshot_digest||'').replace(/^sha256:/,'').slice(0,12))}</small><small>parent ${artifactLineage(artifact).parents.map(esc).join(', ')||'Alpine substrate'}</small>${contentsLine(artifact.snapshot?.contents)}${artifact.enables?.length?`<small>ENABLES <span class="capability t-tag t-supplement t-label">${esc(enabledNames(artifact.enables))}</span>${(artifact.enables||[]).filter(item=>item.name==='acp').map(item=>` · commando <input class="command-input t-field" data-command-layer="${esc(artifact.id)}" value="${esc(item.command||'')}" placeholder="bijv. codex-acp of claude-agent-acp" title="Entrypoint van de ACP-agent in deze laag; achteraf aan te passen, EDIT neemt het over">`).join('')}</small>`:''}${optionsLine}${settingsLine}</div><div class="artifact-actions t-actions t-record-actions">${use}${fetchOptions}${contentsButton(artifact)}${loginsButton(artifact)}${next}${remove}</div></article>`;
  }).join('');
  ...
  root.querySelectorAll('[data-logins-artifact]').forEach(button=>button.onclick=()=>openLogins(button.dataset.loginsArtifact));
  if(document.getElementById('logins-dialog').open)renderLogins();
...
function resetJobForm(source=null){
  const form=document.getElementById('job-form');form.reset();jobAttachmentFiles=[];renderJobAttachmentSelection();forkingJobID=source?.id||'';jobRepositories=source?(source.repositories?.length?source.repositories:[{repository_id:source.git_repository_id,mode:'change',base_ref:source.base_ref}]).map(item=>({repository_id:item.repository_id,mode:item.mode||'change',base_ref:item.base_ref||''})):[];pendingJobSubmission=null;document.getElementById('job-owner').value=currentOperator();document.getElementById('job-forked-from').value=forkingJobID;document.getElementById('job-dialog-title').textContent=source?'Gesloten Job forken':'Nieuwe Job';document.getElementById('job-dialog-description').textContent=source?'Start een vervolg met de eerdere goal, bijlagen en deliverables als context. De repositories van de bron staan er alvast; zet er een op Ter referentie of voeg er een toe om ergens anders verder te werken.':'Kies de flow en geef deze uitvoering één duidelijke goal.';const context=document.getElementById('job-fork-context');context.hidden=!source;context.innerHTML=source?`${icon('fork_right')} Vervolg op <strong>${esc(source.title)}</strong> · basis <code>${esc(source.branch)}</code>`:'';renderGitOptions();renderTemplateOptions();renderEnvironmentOptions();fillMCPSelect(document.getElementById('job-mcp'),source?.mcp_server_ids||[]);if(source){form.elements.title.value=`Vervolg: ${source.title}`;form.elements.reference.value=source.reference||'';form.elements.objective.placeholder='Welke bug, feedback of vervolgvraag moet met deze bestaande context worden opgepakt?';if(snapshot.workflow_templates.some(item=>item.id===source.template_id))form.elements.template_id.value=source.template_id;if(capabilitySelectors('acp').includes(source.environment_selector))form.elements.environment_selector.value=source.environment_selector;}else{form.elements.objective.placeholder='Werkende single switch voor darkmode';}document.getElementById('job-attachments').value='';renderJobAttachmentSelection();setJobStep(1);updateJobRecipeCheck();
}
...
// A Job's repositories are one list: the first row is there from the
// start, + adds another. Each row says what the Job does with it (change
// on the Job branch, or read only) and where it starts. The first row
// that changes is the Job's main repository.
let jobRepositories=[];
function mainJobRepository(){return jobRepositories.find(row=>row.mode!=='reference')||jobRepositories[0]||null;}
function repositoryOptionsHTML(selected){return snapshot.git_repositories.map(repository=>`<option value="${esc(repository.id)}" ${repository.id===selected?'selected':''}>${esc(repository.name)}</option>`).join('');}
function renderJobRepositoryRows(){const root=document.getElementById('job-repositories'),fixed=false,disabled='';
  root.innerHTML=jobRepositories.length?jobRepositories.map((row,index)=>{const repository=byID(snapshot.git_repositories,row.repository_id),duplicate=jobRepositories.some((other,otherIndex)=>otherIndex<index&&other.repository_id===row.repository_id);return `<div class="job-repository-row"><select data-repo-row="${index}" data-repo-field="repository_id" aria-label="Repository" ${disabled}>${repositoryOptionsHTML(row.repository_id)}</select><select data-repo-row="${index}" data-repo-field="mode" aria-label="Wat doet de Job ermee" ${disabled}><option value="change" ${row.mode!=='reference'?'selected':''}>Aanpassen</option><option value="reference" ${row.mode==='reference'?'selected':''}>Ter referentie</option></select><input class="t-field" data-repo-row="${index}" data-repo-field="base_ref" value="${esc(row.base_ref||'')}" placeholder="vanaf ${esc(repository?.default_ref||'default branch')}" aria-label="Start vanaf" ${disabled}>${jobRepositories.length>1&&!fixed?`<button class="icon-button t-action t-icon-button" type="button" data-repo-remove="${index}" aria-label="Weglaten" title="Deze repository weglaten">${icon('close')}</button>`:'<span></span>'}${duplicate?`<span class="form-wide warning">Deze repository staat er al bij.</span>`:''}</div>`;}).join(''):'<span class="hint">Configureer eerst een repository onder Connections → Git.</span>';
  root.querySelectorAll('[data-repo-field]').forEach(field=>field.onchange=()=>{jobRepositories[Number(field.dataset.repoRow)][field.dataset.repoField]=field.value;renderJobRepositoryRows();updateJobRecipeCheck();});
  root.querySelectorAll('[data-repo-remove]').forEach(button=>button.onclick=()=>{jobRepositories.splice(Number(button.dataset.repoRemove),1);renderJobRepositoryRows();updateJobRecipeCheck();});
  document.getElementById('job-repository-add').disabled=fixed||!snapshot.git_repositories.length;}
document.getElementById('job-repository-add').onclick=()=>{const taken=new Set(jobRepositories.map(row=>row.repository_id)),free=snapshot.git_repositories.find(repository=>!taken.has(repository.id))||snapshot.git_repositories[0];if(!free){showError(new Error('Configureer eerst een repository onder Connections → Git.'));return;}jobRepositories.push({repository_id:free.id,mode:jobRepositories.length?'reference':'change',base_ref:''});renderJobRepositoryRows();updateJobRecipeCheck();};
function jobRepositoriesPayload(){return jobRepositories.map(row=>({repository_id:row.repository_id,mode:row.mode||'change',base_ref:String(row.base_ref||'').trim()}));}
// renderGitOptions keeps the rows valid against the repositories that
// exist: a first row appears as soon as there is a repository, a row whose
// repository is gone goes too.
function renderGitOptions(){jobRepositories=jobRepositories.filter(row=>snapshot.git_repositories.some(repository=>repository.id===row.repository_id));if(!jobRepositories.length&&snapshot.git_repositories.length&&!forkingJobID)jobRepositories.push({repository_id:snapshot.git_repositories[0].id,mode:'change',base_ref:''});renderJobRepositoryRows();}
...
function updateJobRecipeCheck(){const template=byID(snapshot.workflow_templates,document.getElementById('job-template').value),repository=byID(snapshot.git_repositories,mainJobRepository()?.repository_id),agentSelector=document.getElementById('environment-selector').value,agent=snapshot.artifacts.find(item=>canUse(item)&&artifactSelector(item)===agentSelector),gitReady=Boolean(template?.git_selector&&capabilityProviderSelectors('git').includes(template.git_selector)),acpReady=Boolean(agent&&artifactEnables(agent,'acp')),layers=[...new Set(jobRepositories.flatMap(row=>byID(snapshot.git_repositories,row.repository_id)?.layer_selectors||[]))],layersReady=layers.every(selector=>snapshot.artifacts.some(item=>canUse(item)&&artifactSelector(item)===selector))&&jobRepositories.some(row=>row.mode!=='reference');document.getElementById('job-recipe-check').innerHTML=`<div class="recipe-part ${gitReady?'ready':'missing'} t-supplement t-label t-well"><strong>${icon(gitReady?'check':'close')} Template → GIT</strong><small>${esc(template?.git_selector||'geen Git environment')}</small></div><div class="recipe-part ${layersReady?'ready':'missing'} t-supplement t-label t-well"><strong>${icon(layersReady?'check':'close')} Repository → tooling</strong><small>${esc(layers.join(' · ')||'geen extra projectlagen')}</small></div><div class="recipe-part ${acpReady?'ready':'missing'} t-supplement t-label t-well"><strong>${icon(acpReady?'check':'close')} Job → ACP</strong><small>${esc(agentSelector||'geen agent')}</small></div>`;document.getElementById('job-submit').disabled=jobSubmitting||!gitReady||!layersReady||!acpReady||!template||!repository;}
...
function renderJobs(){
  ...
  const cards=jobs.map(job=>{
    ...
    const deliverableChips=latestDeliverables(job.id).map(deliverableChip).join(''),attachmentChips=jobAttachments(job.id).map(attachmentHTML).join('');
    const runHTML=runs.map(run=>{
      ...
      const changeAction=allowsChanges?`<button class="small-button t-action" data-job-changes="${esc(job.id)}" data-change-session="${esc(session?.id||'')}" data-change-live="${active&&ready?'true':'false'}" ${canViewStepChanges?'':'disabled'} title="${canViewStepChanges?'Bekijk alleen de changes van deze stap':'Deze stap heeft nog niets op de branch gezet'}">${icon('difference')}Changes</button>`:'';
      const appServices=(repository?.services||[]),appPanel=appServices.length&&active&&ready&&session?`<details class="app-panel t-layer t-card t-disclosure" data-app-session="${esc(session.id)}" data-detail-state="app:${esc(session.id)}" ${detailOpenAttribute(`app:${session.id}`,session.executor==='expose')}><summary>${icon('rocket_launch')}Test-app · ${appServices.map(service=>esc(service.name)).join(', ')}</summary><div class="app-panel-body" data-app-body>Status ophalen…</div></details>`:'';
      // The current step always has a chat: with its running agent, or,
      // when the step gave its capsule up while it waits, with a new
      // capsule that comes when the chat opens.
      const chattable=active&&session&&(ready?acp:session.executor!=='action'&&session.executor!=='expose'),closable=active&&session&&ready&&!stepIsRunning;
      const actions=`${changeAction}${chattable?`<button class="small-button t-action" data-open-acp="${esc(session.id)}" title="${ready?'Chat met de agent van deze stap':'De stap heeft geen capsule nu; de chat start er een (met een vrije login)'}">${icon('chat')}Chat</button>`:''}${closable?`<button class="small-button t-action" data-close-capsule="${esc(session.id)}" title="Sluit de capsule van deze stap; de login die hij vasthoudt komt vrij">${icon('power_settings_new')}Sluit</button>`:''}${retryable?`<button class="small-button t-action" data-retry-session="${esc(session.id)}">${icon('replay')}Retry</button>`:''}`;
      ...
      const answerButton=question?.kind==='agent'&&question.items?.length?`<button class="primary t-action" data-answer-question="${esc(question.id)}"${lock}>${icon('send')}BEANTWOORD</button>`:'';
      const questionButtons=question?.kind==='action'?`<button class="primary t-action" data-accept-question="${esc(question.id)}"${lock}>${icon('replay')}OPNIEUW</button><button class="small-button t-action" data-reject-question="${esc(question.id)}"${lock} title="Volg de REJECT-route naar ${esc(workflowTargetLabel(template,question.reject_target))}">${icon('cancel')}REJECT</button>`:`${answerButton}${acceptButtons(question,answerButton?'small-button':'primary',lock)}<button class="${answerButton?'small-button':'danger t-action--danger'} t-action" data-reject-question="${esc(question?.id)}"${lock}>${icon('cancel')}REJECT</button>${active&&ready&&acp?`<button class="small-button t-action" data-open-acp="${esc(session.id)}">${icon('chat')}CHAT</button>`:''}`;
      const questionHTML=question?`<div class="question-card t-layer t-card" data-workflow-question="${esc(question.id)}"><strong>${workflowDecisionTitle(question)}</strong>${question.kind==='approval'?decisionHTML:agentQuestionFormHTML(question)}${workflowDecisionRoutes(question,template)}${agentWorking?`<small class="question-hint">${icon('progress_activity')}De agent is bezig in de chat · beslissen kan zodra de beurt eindigt</small>`:''}<div class="panel-actions t-actions t-actions--end">${questionButtons}</div></div>`:'';
      const actionHTML=actionResult?(actionResult.type==='git.merge'?`<div class="workflow-result t-layer t-card"><div class="workflow-result-copy">${icon('merge')}<div><strong>Gemerged in ${esc(job.base_ref||'basisbranch')}</strong><small>${esc(actionResult.detail||'')}</small></div></div>${actionResult.url?`<a class="workflow-result-link t-action" href="${esc(actionResult.url)}" target="_blank" rel="noopener noreferrer">Commit ${icon('open_in_new')}</a>`:''}</div>`:`<div class="workflow-result t-layer t-card"><div class="workflow-result-copy">${icon('merge')}<div><strong>Pull request #${esc(actionResult.external_id||'')}</strong><small>Remote resultaat aangemaakt</small></div></div><a class="workflow-result-link t-action" href="${esc(actionResult.url)}" target="_blank" rel="noopener noreferrer">Openen ${icon('open_in_new')}</a></div>`):'';
      const actionFailure=session?.executor==='action'&&run.reject_reason&&!question?`<p class="warning">${esc(run.reject_reason)}</p>`:'';
      const head=`<span class="workflow-marker ${esc(run.status)}"></span><span class="workflow-step-copy"><strong>${esc(run.phase_name)} <span class="tag t-supplement t-label t-tag">poging ${run.attempt}</span>${session?.executor==='action'?'<span class="tag capability t-supplement t-label t-tag">ACTION</span>':''}</strong><small>${session?short(session.id):'geen Session'}</small>${active?presenceHTML:''}</span><span class="workflow-step-status">${esc(sessionPreparation(session)?'voorbereiden':run.status)}</span>`;
      const body=`<div class="workflow-step-body"><div class="workflow-step-content">${outputs.length?`<div class="deliverable-chips">${outputs.map(item=>`<button class="deliverable-chip t-supplement t-hover t-label t-action" data-deliverable="${esc(item.id)}">${esc(item.name)} · r${item.revision}</button>`).join('')}</div>`:''}${actionHTML}${actionFailure}${appPanel}${questionHTML}${question?.kind==='approval'?'':decisionHTML}</div>${actions?`<div class="session-actions t-actions">${actions}</div>`:''}</div>`;
      ...
      return `<details class="workflow-step ${stepIsRunning?'active':''} t-disclosure" data-detail-state="${esc(stepDetailKey)}" ${detailOpenAttribute(stepDetailKey,stepIsRunning)}><summary>${head}</summary>${body}</details>`;
    }).join('');
    const legacySessions=!template?sessions.map(session=>{const composition=byID(snapshot.compositions,session.prepared_composition_id),ready=composition?.runtime&&composition.runtime.status!=='stopped',acp=(composition?.enabled||[]).some(item=>item.name==='acp');return `<div class="session t-card"><div><strong>${esc(session.role||'worker')}</strong><small>${esc(session.git_ref)}</small>${sessionPresenceHTML(session)}</div><div class="session-actions t-actions"><span class="tag t-supplement t-label t-tag">${ready?'capsule ready':esc(session.status)}</span>${ready&&acp?`<button class="primary t-action" data-open-acp="${esc(session.id)}">Open chat</button>`:`<button class="small-button t-action" data-action="use-session" data-id="${esc(session.id)}">Run</button>`}</div></div>`;}).join(''):'';
    const detailKey=`job:${job.id}`,createdAt=formatDateTime(job.created_at);
    const referenced=snapshot.jobs.some(candidate=>candidate.forked_from_job_id===job.id),isOwner=Boolean(currentOperator()),latestTemplate=byID(snapshot.workflow_templates,job.template_id),newerTemplate=!jobIsClosed(job)&&latestTemplate&&template&&(latestTemplate.revision||1)>(template.revision||1)?latestTemplate:null,changeAction=`<button class="small-button t-action" type="button" data-job-changes="${esc(job.id)}" ${hasComparisonWorkspace?'':'disabled'} title="${hasComparisonWorkspace?'Bekijk alle Job changes sinds de basisbranch':'Deze Job heeft nog geen branch'}">${icon('difference')}Changes</button>`,ownerActions=jobIsClosed(job)?`<button class="small-button t-action" type="button" data-fork-job="${esc(job.id)}">${icon('fork_right')}Fork</button>${isOwner?`<button class="icon-button danger t-action t-icon-button t-action--danger" type="button" data-remove-job="${esc(job.id)}" ${referenced?'disabled':''} title="${referenced?'Deze Job levert context aan een vervolg-Job':'Job definitief verwijderen'}" aria-label="Job definitief verwijderen">${icon('delete')}</button>`:''}`:(isOwner?`<button class="small-button t-action" type="button" data-job-environment="${esc(job.id)}" title="Agent/environment en MCP-connecties voor de volgende stappen">${icon('tune')}Omgeving</button>${newerTemplate?`<button class="small-button t-action" type="button" data-adopt-template="${esc(job.id)}" title="Deze Job draait op revisie ${template?.revision||1}; het Template staat op revisie ${newerTemplate.revision}">${icon('upgrade')}Template r${newerTemplate.revision}</button>`:''}<button class="small-button t-action" type="button" data-add-job-attachment="${esc(job.id)}" title="PDF of afbeelding toevoegen">${icon('attach_file')}Bijlage</button><button class="small-button t-action" type="button" data-close-job="${esc(job.id)}">${icon('archive')}Sluiten</button>`:'');
    const source=byID(snapshot.jobs,job.forked_from_job_id);return {key:job.id,html:`<article class="job ${jobIsClosed(job)?'closed':''} t-layer t-card"><div class="job-head"><div><div class="job-title-row"><h3>${esc(job.title)}</h3>${createdAt?`<span class="job-created" title="Aangemaakt ${esc(createdAt)}">${icon('calendar_today')}${esc(createdAt)}</span>`:''}</div><div class="job-objective md t-prose">${job.objective?markdown(job.objective):'<p class="hint">Goal wordt in de brainstorm bepaald.</p>'}</div><div class="meta"><span class="tag ${statusClass} t-supplement t-label t-tag">${esc(workflowStatus)}</span>${jobIsClosed(job)?`<span class="tag t-supplement t-label t-tag">${icon('person')} ${esc(jobAssignee(job))}</span>`:`<select class="assignee-select" data-icon="person" aria-label="Bij wie ligt deze Job" title="Bij wie ligt deze Job" data-assign-job="${esc(job.id)}">${assigneeOptions(job)}</select>`}${job.reference?`<span class="tag reference t-supplement t-label t-tag" title="Referentie">${icon('tag')} ${esc(job.reference)}</span>`:''}${source?`<span class="tag capability t-supplement t-label t-tag">${icon('fork_right')} ${esc(source.title)}</span>`:''}${template?`<span class="tag t-supplement t-label t-tag">${esc(template.name)} · r${template.revision||1}</span>`:''}<span class="tag branch t-supplement t-label t-tag">${esc(job.branch)}</span>${(job.repositories?.length?job.repositories:[{name:repository?.name||'Git ontbreekt',base_ref:job.base_ref,mode:'change'}]).map(item=>`<span class="tag ${item.mode==='reference'?'reference-repo':''} t-supplement t-label t-tag" title="${item.mode==='reference'?'Alleen ter referentie in de capsule':'Wordt aangepast op de Job-branch'}${item.path?` · /workspace/${esc(item.path)}`:''}">${item.mode==='reference'?icon('visibility')+' ':''}${esc(item.name)}@${esc(item.base_ref)}</span>`).join('')}</div>${attachmentChips?`<div class="attachment-selection">${attachmentChips}</div>`:''}${deliverableChips?`<div class="deliverable-chips">${deliverableChips}</div>`:''}</div><div class="job-head-actions t-actions">${changeAction}${ownerActions}</div></div><details class="job-detail t-disclosure" data-job-detail="${esc(job.id)}" data-detail-state="${esc(detailKey)}" ${detailOpenAttribute(detailKey,job.workflow_status==='busy')}><summary>${runs.length||sessions.length} stap${(runs.length||sessions.length)===1?'':'pen'} · workflow en Sessions</summary><div class="job-detail-body">${template?runHTML:`<div class="sessions">${legacySessions}</div>`}</div></details></article>`};
  });
...
function appServiceRowHTML(service,live){
  ...
  return `<div class="app-service"><span class="app-dot ${cls}"></span><strong>${esc(service.name)}</strong><span class="app-status">${esc(status)}${live?.error&&live.status!=='running'?` · ${esc(live.error)}`:''}</span><span class="app-links">${links}</span>${live?`<button class="small-button t-action" type="button" data-app-logs="${esc(service.name)}">${icon('subject')}Logs</button>`:''}</div>`;
}
async function refreshAppPanel(panel){
  ...
  try{
    ...
    body.innerHTML=`${head}${status.services.map(service=>appServiceRowHTML(service,(status.running||[]).find(item=>item.service===service.name))).join('')}<div class="space-before-md panel-actions t-actions t-actions--end"><button class="primary t-action" type="button" data-app-start ${status.start==='running'?'disabled':''}>${icon('play_arrow')}${anyRunning?'Herstart':'Start'}</button>${anyRunning?`<button class="danger t-action t-action--danger" type="button" data-app-stop>${icon('stop')}Stop</button>`:''}</div>`;
...
function renderTemplates(){const root=region('templates');if(!snapshot.workflow_templates.length){root.innerHTML='<div class="empty t-empty">Nog geen Templates. Maak bijvoorbeeld Ontwikkeling met Ontwerp → Ontwikkelen → Review.</div>';return;}root.innerHTML=snapshot.workflow_templates.map(template=>`<article class="template-card t-card"><div class="job-head"><div><h3>${esc(template.name)} <span class="tag t-supplement t-label t-tag">r${template.revision||1}</span>${template.git_selector?` <span class="tag capability t-supplement t-label t-tag">GIT · ${esc(template.git_selector)}</span>`:''}</h3><p>${esc(template.description||'Eigen workflow')}</p></div><div class="panel-actions t-actions t-actions--end">${template.created_by===currentOperator()?`<button class="small-button t-action" data-edit-template="${esc(template.id)}">${icon('edit')}Nieuwe revisie</button>`:''}<button class="small-button t-action" data-copy-template="${esc(template.id)}" title="Nieuw Template met dezelfde stappen">${icon('content_copy')}Kopie</button>${template.created_by===currentOperator()?`<button class="danger t-action t-action--danger" data-remove-template="${esc(template.id)}">${icon('delete')}Verwijder</button>`:''}</div></div><div class="template-flow">${template.phases.map((phase,index)=>`${index?'<span class="phase-arrow material-symbols-outlined t-icon" aria-hidden="true">arrow_forward</span>':''}<span class="phase-pill t-tag t-supplement t-label">${index+1}. ${esc(phase.name)}${phase.executor==='action'?' · ACTION':phase.environment_selector?' · '+esc(phase.environment_selector):' · JOB DEFAULT'}${phase.allow_changes||phase.allow_commit?' · WRITE':''}${(phase.inject||[]).length?' · IN '+esc(phase.inject.join('+')):''}${phase.accept?.ask_user?' · A:USER':''}${phase.reject?.ask_user?' · R:USER':''}</span>`).join('')}</div></article>`).join('');root.querySelectorAll('[data-edit-template]').forEach(button=>button.onclick=()=>openTemplateEditor(button.dataset.editTemplate));root.querySelectorAll('[data-copy-template]').forEach(button=>button.onclick=()=>openTemplateCopy(button.dataset.copyTemplate));root.querySelectorAll('[data-remove-template]').forEach(button=>button.onclick=()=>removeTemplate(button.dataset.removeTemplate));}
...
// performs itself.
const stepKinds=[['agent','Agent · ACP-sessie'],['resolve_merge','AI merge · agentstap die de basisbranch in de Job-branch merget; Spin zet de merge klaar met de conflicten erin'],['expose','Test-app · start de services van de repository en wacht op jouw oordeel'],['merge','Mergen · Spin merget de Job-branch in de basisbranch; een conflict is een REJECT'],['pull_request','Pull request · Spin maakt een pull request op de remote']];
function stepKindOf(values){if(values.executor==='action')return values.action?.type==='git.merge'?'merge':'pull_request';if(values.resolve_merge)return 'resolve_merge';return values.executor||'agent';}
function stepKindFields(kind){if(kind==='merge'||kind==='pull_request')return {executor:'action',action:{type:kind==='merge'?'git.merge':'git.pull_request.create'}};if(kind==='resolve_merge')return {executor:'agent',resolve_merge:true};return {executor:kind};}
function refreshTemplateInjections(){const steps=[...document.querySelectorAll('[data-template-step]')];steps.forEach((step,index)=>{const root=step.querySelector('[data-inject-deliverables]'),available=new Map();steps.slice(0,index).forEach(previous=>previous.querySelectorAll('[name=deliverable_name]').forEach(input=>{const name=input.value.trim(),key=name.toLowerCase();if(name&&!available.has(key))available.set(key,name);}));if(!step._injectSelection)step._injectSelection=new Set();root.innerHTML=available.size?[...available].map(([key,name])=>`<label class="inject-option t-check-row"><input class="t-field" type="checkbox" name="inject_deliverable" value="${esc(name)}" ${step._injectSelection.has(key)?'checked':''}> ${esc(name)}</label>`).join(''):'<span class="hint">Nog geen deliverables uit eerdere stappen.</span>';root.querySelectorAll('[name=inject_deliverable]').forEach(input=>input.onchange=()=>{const key=input.value.trim().toLowerCase();if(input.checked)step._injectSelection.add(key);else step._injectSelection.delete(key);});});}
...
function addDeliverableRow(container,values={}){const row=document.createElement('div');row.className='deliverable-row';row.innerHTML=`<input class="t-field" name="deliverable_name" required placeholder="FO" value="${esc(values.name||'')}"><input class="t-field" name="deliverable_description" placeholder="Functioneel ontwerp" value="${esc(values.description||'')}"><select name="deliverable_kind" title="Wat de agent moet opleveren; Spin controleert het bij het opleveren">${deliverableKinds.map(([value,label])=>`<option value="${value}" ${(values.kind||'markdown')===value?'selected':''}>${esc(label)}</option>`).join('')}</select><label class="flush-margin"><input class="intrinsic-width t-field" name="deliverable_required" type="checkbox" ${values.required===false?'':'checked'}> verplicht</label><button class="icon-button t-action t-icon-button" type="button" title="Verwijder">${icon('delete')}</button>`;row.querySelector('button').onclick=()=>{row.remove();refreshTemplateInjections();};row.querySelector('[name=deliverable_name]').oninput=refreshTemplateInjections;container.appendChild(row);refreshTemplateInjections();}
function addTemplateStep(values={}){
  ...
  step.className='template-step-editor t-panel';step.dataset.templateStep='';step.dataset.stepId=id;
  step._injectSelection=new Set((values.inject||[]).map(name=>String(name).trim().toLowerCase()));
  step.innerHTML=`<div class="template-step-head t-toolbar"><strong>Stap <span data-step-number></span></strong><button class="danger t-action t-action--danger" type="button" data-remove-step>${icon('delete')}Verwijder stap</button></div><div class="template-step-body"><div><label>Naam</label><input class="t-field" name="phase_name" required placeholder="Ontwerp" value="${esc(values.name||'')}"></div><div><label>Soort</label><select name="phase_executor">${stepKinds.map(([value,label])=>`<option value="${value}" ${stepKindOf(values)===value?'selected':''}>${esc(label)}</option>`).join('')}</select></div><div><label>Environment</label><select name="phase_environment"></select><small>Leeg gebruikt de agent van de Job (daar kies je het model). Een laag zonder agent, zoals tool:git, komt als extra laag bij die agent.</small></div><div class="wide template-runtime"><div><label>Extra lagen</label><select name="phase_with" multiple size="4"></select><small>Alleen deze stap krijgt deze extra lagen.</small></div><div><label>Beleid</label><label class="inline-option t-check-row"><input class="intrinsic-width t-field" name="allow_changes" type="checkbox" ${values.allow_changes||values.allow_commit?'checked':''}> wijzigingen gaan mee bij ACCEPT</label><small>Aan: Spin vouwt de wijzigingen bij definitieve ACCEPT tot één commit op de Job-branch. Uit: de agent mag in zijn workspace bouwen en experimenteren, maar niets gaat mee en ACCEPT bevestigt alleen de basis.</small></div><div class="wide template-runtime"><div><label>Model</label><input class="t-field" name="phase_model" list="agent-model-options" placeholder="agent default" value="${esc(values.model||'')}" autocomplete="off"><small>Uit de opties van de ACP-laag (Environments → Opties ophalen). Leeg laat de agent kiezen.</small></div><div><label>Reasoning</label><input class="t-field" name="phase_reasoning" list="agent-effort-options" placeholder="agent default" value="${esc(values.reasoning_effort||'')}" autocomplete="off"><small>Denkniveau voor deze stap, bijvoorbeeld low voor routinewerk.</small></div></div><div class="wide"><label>Omschrijving / opdracht voor de agent</label><textarea class="t-field" name="instructions" data-markdown required placeholder="Je maakt het functioneel en technisch ontwerp.">${esc(values.instructions||'')}</textarea></div><div class="wide"><label>Context injecteren</label><div class="inject-editor" data-inject-deliverables></div><small>De Job Goal gaat altijd mee. Aangevinkte documenten worden verplicht als laatste revisie opgenomen.</small></div><div class="wide"><label>Deliverables opleveren</label><div class="deliverable-editor" data-deliverables></div><button class="space-before-sm small-button t-action" type="button" data-add-deliverable>${icon('add')}Document</button></div></div><div class="wide"><label>Overgangen</label><div class="transition-editor"><div class="transition-route"><label>ACCEPT →</label><select name="accept_target" data-transition-target="accept"></select><label class="transition-gate t-check-row"><input class="t-field" name="accept_ask_user" type="checkbox" ${(values.accept?.ask_user??values.ask_user)?'checked':''}> Ask user</label></div><div class="transition-route"><label>REJECT →</label><select name="reject_target" data-transition-target="reject"></select><label class="transition-gate t-check-row"><input class="t-field" name="reject_ask_user" type="checkbox" ${(values.reject?.ask_user??values.ask_user)?'checked':''}> Ask user</label></div><div><label>REJECT max</label><input class="t-field" name="reject_max" type="number" min="0" value="${values.reject?.max??2}"></div><div><label>Na maximum</label><div class="section-inset hint">ASK USER</div></div></div></div></div>`;
  root.appendChild(step);mountMarkdown(step);(values.deliverables||[]).forEach(item=>addDeliverableRow(step.querySelector('[data-deliverables]'),item));
...
function collectTemplatePhases(){return [...document.querySelectorAll('[data-template-step]')].map(step=>({id:step.dataset.stepId,name:step.querySelector('[name=phase_name]').value,...stepKindFields(step.querySelector('[name=phase_executor]').value),instructions:step.querySelector('[name=instructions]').value,model:step.querySelector('[name=phase_model]').value.trim(),reasoning_effort:step.querySelector('[name=phase_reasoning]').value.trim(),environment_selector:step.querySelector('[name=phase_environment]').value,with_selectors:selectedValues(step.querySelector('[name=phase_with]')),allow_changes:step.querySelector('[name=allow_changes]').checked||step.querySelector('[name=phase_executor]').value==='resolve_merge',inject:[...step.querySelectorAll('[name=inject_deliverable]:checked')].map(input=>input.value),deliverables:[...step.querySelectorAll('.deliverable-row')].map(row=>({name:row.querySelector('[name=deliverable_name]').value,description:row.querySelector('[name=deliverable_description]').value,kind:row.querySelector('[name=deliverable_kind]').value,required:row.querySelector('[name=deliverable_required]').checked})),accept:{target:step.querySelector('[name=accept_target]').value,ask_user:step.querySelector('[name=accept_ask_user]').checked},reject:{target:step.querySelector('[name=reject_target]').value,ask_user:step.querySelector('[name=reject_ask_user]').checked,max:Number(step.querySelector('[name=reject_max]').value||0),exhausted:'ASK_USER'}}));}

function addServiceRow(container,values={}){
  const row=document.createElement('div');row.className='service-row t-card';
  row.innerHTML=`<div class="service-row-head"><input class="t-field" name="service_name" required placeholder="web" value="${esc(values.name||'')}" title="Servicenaam (letters, cijfers, streepjes)"><input class="t-field" name="service_ports" placeholder="poorten, bv. 3000 5173" value="${esc((values.ports||[]).join(' '))}"><input class="t-field" name="service_env" placeholder="env-naam (var/env/<naam>.env)" value="${esc(values.env||'')}"><button class="icon-button t-action t-icon-button" type="button" title="Verwijder">${icon('delete')}</button></div><div class="service-row-body"><div><label>Voorbereiden (één commando per regel)</label><textarea class="t-field" name="service_prepare" rows="2" placeholder="npm ci">${esc((values.prepare||[]).join('\n'))}</textarea></div><div><label>Run</label><input class="t-field" name="service_run" placeholder="npm run start" value="${esc(values.run||'')}"><label class="space-before-xs">…of image (dependency)</label><input class="t-field" name="service_image" placeholder="postgres:16" value="${esc(values.image||'')}"></div></div>`;
...
function renderGit(){
  ...
  if(!snapshot.git_repositories.length){root.innerHTML='<div class="empty t-empty">Nog geen Git repositories. Maak eerst tool:git en voeg daarna een remote toe.</div>';return;}
  root.innerHTML=snapshot.git_repositories.map(repository=>{
    ...
    const access=scope==='public'?'<span class="tag t-supplement t-label t-tag">public · geen credentials</span>':account?`<span class="tag secret t-supplement t-label t-tag">${esc(scopeLabel)} → ${esc(gitAccountLabel(account))}</span>`:`<span class="tag warning t-supplement t-label t-tag">${esc(scopeLabel)} · ${esc(gitRemoteHost(repository.remote_url))} niet gekoppeld</span>`;
    const actions=repository.created_by===currentOperator()?`<div class="panel-actions repo-secondary t-panel-actions t-actions t-actions--end"><button class="small-button t-action" type="button" data-edit-git="${esc(repository.id)}">${icon('edit')}Bewerken</button><button class="danger t-action t-action--danger" type="button" data-remove-git="${esc(repository.id)}">${icon('delete')}Verwijderen</button></div>`:'';
    return `<article class="repo-card t-layer t-record"><div class="t-record-content"><div class="artifact-title"><strong>${esc(repository.name)}</strong><span class="tag branch t-supplement t-label t-tag">${esc(repository.provider||gitRemoteHost(repository.remote_url))}</span><span class="tag branch t-supplement t-label t-tag">${esc(repository.default_ref)}</span>${access}</div><small>${esc(repository.remote_url)}</small><small>Projectlagen: ${selectedLayers.map(esc).join(' · ')||'geen'}</small><small>Test-app: ${(repository.services||[]).length?repository.services.map(service=>`${esc(service.name)}${service.ports?.length?' :'+service.ports.map(esc).join('/'):''}`).join(' · '):'geen services'}${repository.service_hosts?.length?` · hosts: ${repository.service_hosts.map(esc).join(', ')}`:''}</small><small>Job: jobs/&lt;name-id&gt;/main · Sessions: jobs/&lt;name-id&gt;/sessions/&lt;id&gt;</small></div><div class="repo-controls">${actions}</div></article>`;
  }).join('');
...
function renderGitAccounts(){
  ...
  providers.innerHTML=snapshot.git_oauth_providers.filter(provider=>provider.configured).map(provider=>`<button class="primary t-action" type="button" data-oauth-provider="${esc(provider.id)}" data-oauth-scope="user">Koppel ${esc(provider.name)}</button>${admin?`<button class="small-button t-action" type="button" data-oauth-provider="${esc(provider.id)}" data-oauth-scope="global">${esc(provider.name)} global</button>`:''}`).join('');
  ...
  configurations.innerHTML=snapshot.git_oauth_providers.map(provider=>{
    const state=provider.configured?`<span class="tag capability t-supplement t-label t-tag">actief · ${esc(provider.source)}</span>`:'<span class="tag t-supplement t-label t-tag">niet ingesteld</span>';
    ...
    return `<form class="form-grid oauth-config-form" data-provider="${esc(provider.id)}"><div class="wide"><div class="artifact-title"><strong>${esc(provider.name)}</strong>${state}</div><p class="space-before-sm hint">${instructions}</p></div><div><label>Client ID</label><input class="t-field" name="client_id" required value="${esc(provider.client_id||'')}" autocomplete="off"></div><div><label>Client secret${provider.configured?' (opnieuw invoeren om te wijzigen)':''}</label><input class="t-field" name="client_secret" type="password" required autocomplete="new-password"></div><div class="wide panel-actions t-actions t-actions--end"><button class="small-button t-action" type="submit">${provider.configured?'Wijzig':'Configureer'} ${esc(provider.name)}</button>${provider.configured?`<button class="danger t-action t-action--danger" type="button" data-remove-oauth="${esc(provider.id)}">Configuratie verwijderen</button>`:''}</div></form>`;
  }).join('');
  ...
  root.innerHTML=accounts.length?accounts.map(account=>{
    ...
    const validity=expired?`<span class="tag warning t-supplement t-label t-tag">verlopen ${esc(new Date(account.expires_at).toLocaleString())} · vernieuwt bij gebruik, anders opnieuw koppelen</span>`:account.expires_at?'OAuth · verloopt '+esc(new Date(account.expires_at).toLocaleString()):'Token · geen vervaldatum';
    const reconnect=oauth&&(scope==='user'?account.operator===currentOperator():authState.user?.role==='admin')?`<button class="small-button t-action" data-reconnect-git-account="${esc(account.provider)}" data-reconnect-scope="${esc(scope)}">Koppel opnieuw</button>`:'';
    return `<article class="git-account-card t-layer t-card"><div><div class="artifact-title"><strong>${esc(account.provider)}:${esc(account.login)}</strong><span class="tag secret t-supplement t-label t-tag">${scope==='global'?'global':`user:${account.operator}`}</span></div><small>${esc(account.host)} · ${esc(account.name||account.login)}${account.email?' · '+esc(account.email):''}</small><small>${validity} · secret redacted</small></div><div class="panel-actions t-actions t-actions--end">${reconnect}${account.operator===currentOperator()?`<button class="danger t-action t-action--danger" data-remove-git-account="${esc(account.id)}">Ontkoppel</button>`:''}</div></article>`;}).join(''):'<div class="empty t-empty">Nog geen persoonlijke of gedeelde Git identities.</div>';
...
function renderMCP(){
  ...
  if(!servers.length){root.innerHTML='<div class="empty t-empty">Nog geen persoonlijke MCP-configuraties.</div>';return;}
  root.innerHTML=servers.map(server=>`<article class="mcp-card t-card"><div><div class="artifact-title"><span>${esc(server.name)}</span><span class="tag t-supplement t-label t-tag">${esc(server.transport)}</span><span class="tag secret t-supplement t-label t-tag">user:${esc(server.operator)}</span></div><small>${server.transport==='stdio'?esc(server.command)+' '+(server.args||[]).map(esc).join(' '):esc(server.url)}</small><small>${(server.env||[]).length} env credentials · ${(server.headers||[]).length} header credentials · values redacted</small></div><button class="danger t-action t-action--danger" data-remove-mcp="${esc(server.id)}">Remove</button></article>`).join('');
...
async function rotateRunnerToken(){if(!await TactileDialog.confirm('Nieuw worker-token maken? Alle runners moeten daarna met het nieuwe token opnieuw starten.'))return;try{const result=await api('/api/runners/token',{method:'POST'});runnerToken=result.token||'';renderRunnerToken();showNotice('Nieuw worker-token gemaakt; geef het aan de runners.');}catch(error){showError(error);}}
function renderRunnerToken(){
  ...
  root.innerHTML=runnerToken?`<code class="runner-token-value">${esc(runnerToken)}</code><button class="small-button t-action" id="hide-runner-token">${icon('visibility_off')}Verberg</button><button class="small-button t-action" id="rotate-runner-token">${icon('autorenew')}Vernieuw</button>`:`<button class="small-button t-action" id="show-runner-token">${icon('key')}Toon worker-token</button>`;
...
function renderRunnerDownloads(){
  ...
  root.innerHTML=`<div class="runner-download-row">${builds.map(([asset,label,glyph])=>`<a class="small-button t-action" href="${base}/${asset}" download="${asset}">${icon(glyph)}${esc(label)}</a>`).join('')}<small>${version?`versie ${esc(version)}`:'laatste release'} · Docker Desktop of Docker Engine vereist</small></div><details class="runner-howto t-disclosure"><summary>Zo start je de runner</summary><pre>chmod +x spin-client-*
mkdir -p var && printf '%s' '&lt;worker-token&gt;' &gt; var/spin-worker.token
./spin-client-&lt;platform&gt; -server ${esc(location.origin)} -name $(hostname)</pre><div class="runner-token t-well" id="runner-token"></div><small>Het worker-token hoort bij deze Spin en staat in zijn database; een admin ziet het hierboven en kan het vernieuwen. Opties: <code>-env-dir</code> voor de env-bestanden van de test-app, <code>-advertise-host</code> voor het adres in de test-app-links, <code>-max-workloads</code> voor het aantal gelijktijdige capsules.</small></details>`;
  renderRunnerToken();
}
...
function renderRunners(){
  ...
  if(!snapshot.clients.length){root.innerHTML='<div class="empty t-empty">Nog geen runner aangemeld. Start spin-client met de server-URL en het worker-token.</div>';return;}
  root.innerHTML=snapshot.clients.map(client=>{
    ...
    const statusClass=client.status==='online'?'capability':(client.draining||client.status==='draining'?'warning':''),admin=authState.user?.role==='admin',drainAction=admin?client.draining?`<button class="small-button t-action" data-resume-client="${esc(client.id)}">${icon('play_arrow')}Hervat</button>`:`<button class="small-button t-action" data-drain-client="${esc(client.id)}">${icon('pause')}Drain</button>`:'',removeAction=admin&&client.status==='offline'&&!active.length?`<button class="small-button t-action" data-remove-client="${esc(client.id)}">${icon('delete')}Verwijder</button>`:'';
    return `<article class="mcp-card t-card"><div><div class="artifact-title"><strong>${esc(client.name)}</strong><span class="tag ${statusClass} t-supplement t-label t-tag">${esc(client.status||'offline')}${client.draining?' · drained':''}</span><span class="tag t-supplement t-label t-tag">${esc(engine.driver||'unknown engine')}</span></div><small>${esc(client.capabilities?.os||'?')}/${esc(client.capabilities?.arch||'?')} · ${esc(client.id)} · max ${esc(client.capabilities?.max_workloads||'∞')} workloads</small><small>${active.length} actief · ${pinned.length} Sessions gekoppeld · laatst gezien ${esc(formatDateTime(client.last_seen_at))}${client.status==='offline'?` · ${esc(elapsedSince(client.last_seen_at))} geleden`:''}</small></div><div class="panel-actions t-actions t-actions--end">${drainAction}${removeAction}</div></article>`;
  }).join('');
...
function renderAccess(){
  ...
  root.innerHTML=snapshot.users.length?snapshot.users.map(user=>{const archived=Boolean(user.archived_at),self=user.id===authState.user?.id,action=admin?`${archived?'':`<button class="small-button t-action" data-password-user="${esc(user.id)}" data-user-label="${esc(user.display_name||user.username)}" title="Nieuw tijdelijk wachtwoord geven">${icon('key')}Wachtwoord</button>`}${self?'':archived?`<button class="small-button t-action" data-restore-user="${esc(user.id)}">${icon('restore')}Herstel</button>`:`<button class="danger t-action t-action--danger" data-archive-user="${esc(user.id)}">${icon('archive')}Archiveer</button>`}`:'';return `<article class="mcp-card ${archived?'archived':''} t-card"><div><div class="artifact-title"><strong>${esc(user.display_name||user.username)}</strong><span class="tag ${user.role==='admin'?'secret':''} t-supplement t-label t-tag">${esc(user.role)}</span>${self?'<span class="tag capability t-supplement t-label t-tag">you</span>':''}${archived?'<span class="tag warning t-supplement t-label t-tag">gearchiveerd</span>':''}</div><small>@${esc(user.username)} · created ${esc(new Date(user.created_at).toLocaleDateString())}${archived?` · archived ${esc(formatDateTime(user.archived_at))}`:''}</small></div><div class="panel-actions t-actions t-actions--end">${action}</div></article>`;}).join(''):'<div class="empty t-empty">Nog geen gebruikers.</div>';
...
async function renderReplicaGenerations(){
  ...
  root.hidden=false;
  if(!root.dataset.loaded){root.innerHTML=`<button class="small-button t-action" id="load-generations">${icon('history')}Herstelpunten uit de replica</button>`;document.getElementById('load-generations').onclick=loadReplicaGenerations;}
}
...
async function loadReplicaGenerations(){
  ...
  try{
    ...
    root.innerHTML=points.length?`<p class="hint">${icon('history')} Herstelpunten in de replica, nieuwste eerst: elke sync van het laatste kwartier, kwartieren van de laatste twee uur, uren van de laatste dag, dagen van de laatste week en de wekelijkse generaties van de laatste maand. Een herstel vervangt de huidige database door de staat van dat moment.</p><div class="restore-points t-table-scroll">${points.slice(0,400).map((point,index)=>`<div class="binding"><span>${esc(formatDateTime(point.at))}</span><span><span class="tag t-supplement t-label t-tag">${esc(pointLevelLabel(point.level))}</span>${index===0&&point.current?'<span class="tag capability t-supplement t-label t-tag">nu</span>':`<button class="small-button t-action" data-restore-point="${index}" data-generation="${esc(point.generation)}" data-at="${esc(point.at)}" data-label="${esc(formatDateTime(point.at))}">${icon('restore')}Herstel</button>`}</span></div>`).join('')}</div>`:'<p class="hint">Nog geen complete generatie in de replica.</p>';
...
async function announceRestoreComplete(result){await TactileDialog.notice(`Restore compleet: ${result.jobs} Jobs, ${result.templates} Templates, ${result.deliverables} deliverables, ${result.attachments} bijlagen en ${result.snapshots} Docker-snapshots. Log opnieuw in.`);location.reload();}
...
async function removeArtifact(id,label){
  ...
  if(!await TactileDialog.confirm(lines.join('\n\n')))return;
...
async function removeMCP(id){if(!await TactileDialog.confirm('Remove deze persoonlijke MCP-configuratie?'))return;try{await api(`/api/mcp-servers/${encodeURIComponent(id)}`,{method:'DELETE'});await refresh(true);}catch(error){showError(error);}}
async function removeGitAccount(id){if(!await TactileDialog.confirm('Deze Git identity ontkoppelen? Repositories lossen daarna automatisch een andere identity voor dezelfde host en scope op, of wachten op een nieuwe koppeling.'))return;try{await api(`/api/git/accounts/${encodeURIComponent(id)}`,{method:'DELETE'});await refresh(true);}catch(error){showError(error);}}
async function removeGit(id){if(!await TactileDialog.confirm('Remove deze Git repositoryconfiguratie? Bestaande Jobs blokkeren dit.'))return;try{await api(`/api/git/repositories/${encodeURIComponent(id)}`,{method:'DELETE'});await refresh(true);}catch(error){showError(error);}}
async function removeTemplate(id){if(!await TactileDialog.confirm('Dit Workflow Template verwijderen? Bestaande Jobs blokkeren dit.'))return;try{await api(`/api/workflow-templates/${encodeURIComponent(id)}`,{method:'DELETE'});await refresh(true);}catch(error){showError(error);}}
async function setUserArchived(id,archived){const user=byID(snapshot.users,id);if(!user)return;if(archived&&!await TactileDialog.confirm(`Gebruiker “${user.display_name||user.username}” archiveren? Alle actieve logins worden ingetrokken; historie en gekoppelde records blijven bestaan.`))return;try{await api(`/api/auth/users/${encodeURIComponent(id)}/${archived?'archive':'restore'}`,{method:'POST'});await refresh(true);}catch(error){showError(error);}}
...
async function closeJob(button){const id=button.dataset.closeJob,job=byID(snapshot.jobs,id);if(!job||!await TactileDialog.confirm(`Job “${job.title}” sluiten? De volledige historie blijft bewaard, maar actieve Sessions en capsules stoppen.`))return;button.disabled=true;const previous={status:job.status,workflow_status:job.workflow_status,current_phase_run_id:job.current_phase_run_id};job.status='cancelled';job.workflow_status='done';job.current_phase_run_id='';renderJobs();try{await api(`/api/jobs/${encodeURIComponent(id)}/close`,{method:'POST'});await refresh(true);}catch(error){Object.assign(job,previous);renderJobs();showError(error);}}
async function removeJob(id){const job=byID(snapshot.jobs,id);if(!job||!await TactileDialog.confirm(`Job “${job.title}” definitief verwijderen? Sessions en lokale containers verdwijnen; de remote Git-branches blijven bestaan.`))return;try{await api(`/api/jobs/${encodeURIComponent(id)}`,{method:'DELETE'});await refresh(true);}catch(error){showError(error);}}
// A retry is a fork at the point where it went wrong: the workspace stays,
// a fresh agent starts with a note on what should go differently.
let pendingRetry=null;
function retrySession(button){const id=button.dataset.retrySession;if(!id)return;pendingRetry={id,button,fromChat:button.id==='chat-retry',transcript:null};document.getElementById('retry-form').reset();document.getElementById('retry-transcript-hint').hidden=true;document.getElementById('retry-title').textContent='Opnieuw met een verse agent';openDialog('retry-dialog');requestAnimationFrame(()=>document.getElementById('retry-note').focus());}
// Editing one of your messages is a fork at that point: the messages before
// it go along as context, the edited text is the new instruction, a fresh
// agent picks up in the kept workspace.
function forkFromMessage(node){const retry=document.getElementById('chat-retry');if(!chatState.sessionID||retry.hidden){showError(new Error('Alleen de actieve stap kan vanaf een bericht opnieuw worden gestart.'));return;}const transcript=[];for(const message of document.querySelectorAll('#chat-feed .chat-message')){if(message===node)break;const text=message.querySelector('.md')?.dataset.source||'';if(text.trim())transcript.push({role:message.classList.contains('user')?'user':'agent',text});}
  pendingRetry={id:chatState.sessionID,button:retry,fromChat:true,transcript};document.getElementById('retry-form').reset();document.getElementById('retry-note').value=node.querySelector('.md')?.dataset.source||'';const hint=document.getElementById('retry-transcript-hint');hint.hidden=false;hint.textContent=transcript.length?`Het gesprek tot dit bericht (${transcript.length} bericht${transcript.length===1?'':'en'}) gaat mee als context; wat erna kwam vervalt.`:'Dit was het eerste bericht; de nieuwe poging begint hiermee.';document.getElementById('retry-title').textContent='Vanaf dit bericht verder';openDialog('retry-dialog');requestAnimationFrame(()=>document.getElementById('retry-note').focus());}
document.getElementById('retry-form').onsubmit=async event=>{event.preventDefault();if(!pendingRetry)return;const {id,button,fromChat,transcript}=pendingRetry,note=String(new FormData(event.target).get('note')||'').trim();pendingRetry=null;closeDialog('retry-dialog');const label=button.innerHTML;button.disabled=true;button.textContent='Opnieuw starten…';try{await api(`/api/sessions/${encodeURIComponent(id)}/retry`,{method:'POST',body:JSON.stringify({note,transcript:transcript||undefined})});if(fromChat)closeDialog('chat-dialog');showNotice('Stap opnieuw gestart met een verse agent; de workspace is bewaard.');await refresh(true);}catch(error){button.disabled=false;button.innerHTML=label;showError(error);}};
...
// Attachments for a new Job gather in a list: dropped on the zone, picked
// with the file dialog, or both, and each one can go again before the Job
// is shot.
let jobAttachmentFiles=[];
function selectedJobAttachmentFiles(){return [...jobAttachmentFiles];}
function attachmentFileAccepted(file){return file.type==='application/pdf'||/^image\/(png|jpeg|webp|gif)$/.test(file.type)||/\.(pdf|png|jpe?g|webp|gif)$/i.test(file.name);}
function addJobAttachmentFiles(files){const rejected=[];for(const file of files){if(!attachmentFileAccepted(file)){rejected.push(file.name);continue;}if(jobAttachmentFiles.some(item=>item.name===file.name&&item.size===file.size))continue;jobAttachmentFiles.push(file);}if(rejected.length)showError(new Error(`Alleen PDF of afbeelding: ${rejected.join(', ')} overgeslagen`));renderJobAttachmentSelection();}
function droppedFiles(event){return [...(event.dataTransfer?.files||[])];}
function bindDropTarget(target,onFiles,className='over'){if(!target||target._dropBound)return;target._dropBound=true;target.addEventListener('dragover',event=>{if(!event.dataTransfer?.types?.includes('Files'))return;event.preventDefault();event.dataTransfer.dropEffect='copy';target.classList.add(className);});target.addEventListener('dragleave',event=>{if(!target.contains(event.relatedTarget))target.classList.remove(className);});target.addEventListener('drop',event=>{if(!event.dataTransfer?.types?.includes('Files'))return;event.preventDefault();event.stopPropagation();target.classList.remove(className);const files=droppedFiles(event);if(files.length)onFiles(files);});}
...
function renderJobAttachmentSelection(){const files=selectedJobAttachmentFiles(),root=document.getElementById('job-attachment-selection');const chips=files.map((file,index)=>`<span class="attachment-chip t-supplement t-label">${icon(file.type==='application/pdf'?'picture_as_pdf':'image')}<span class="attachment-chip-name">${esc(file.name)}</span><span class="attachment-size">${esc(formatBytes(file.size))}</span><button type="button" class="chip-remove t-remove t-action t-icon-button" data-remove-attachment="${index}" title="Deze bijlage weglaten" aria-label="Weglaten">${icon('close')}</button></span>`).join('');let warning='';try{validateJobAttachmentFiles(files);}catch(error){warning=`<span class="warning">${esc(error.message)}</span>`;}root.innerHTML=chips+warning;root.querySelectorAll('[data-remove-attachment]').forEach(button=>button.onclick=()=>{jobAttachmentFiles.splice(Number(button.dataset.removeAttachment),1);renderJobAttachmentSelection();});}
...
document.getElementById('diff-full-toggle').onclick=()=>{fullFileState.chat=!fullFileState.chat;if(chatState.selectedDiffPath)openDiff(chatState.selectedDiffPath);};
document.getElementById('job-change-full-toggle').onclick=()=>{fullFileState.job=!fullFileState.job;if(jobChangesState.selectedPath)openJobChangeDiff(jobChangesState.selectedPath);};
...
document.getElementById('record-presets').innerHTML=recordPresets.map((preset,index)=>`<button class="quick t-action" type="button" data-record-preset="${index}">${esc(preset.label)}</button>`).join('');
...
['job-template','environment-selector'].forEach(id=>document.getElementById(id).addEventListener('change',updateJobRecipeCheck));
document.getElementById('job-attachments').onchange=event=>{addJobAttachmentFiles([...event.target.files]);event.target.value='';};
{const zone=document.getElementById('job-dropzone'),input=document.getElementById('job-attachments');document.getElementById('choose-job-attachments').onclick=()=>input.click();bindDropTarget(zone,addJobAttachmentFiles);}
