...
function renderDeliverableRevision(id){
  ...
  if(visual){const previewURL=`/preview/${encodeURIComponent(item.id)}/`,shape=item.bundle.entry==='index.html'?`${item.bundle.files} bestanden`:item.bundle.content_type;content.innerHTML=`<div class="preview-bar"><span class="hint">${esc(shape)} · ${esc(formatBytes(item.bundle.size))}</span><a class="small-button" href="${previewURL}" target="_blank" rel="noopener">${icon('open_in_new')}Open in tabblad</a></div><iframe class="preview-frame" src="${previewURL}" sandbox="allow-scripts allow-forms allow-modals" referrerpolicy="no-referrer" title="${esc(item.name)} revisie ${item.revision}"></iframe>`;richContentReady=Promise.resolve();content.onmouseup=null;lock.textContent=isLatest?'Laatste revisie · commentaar geldt voor de hele preview':'Historische revisie · alleen lezen';}
...
function addDeliverableRow(container,values={}){const row=document.createElement('div');row.className='deliverable-row';row.innerHTML=`<input name="deliverable_name" required placeholder="FO" value="${esc(values.name||'')}"><input name="deliverable_description" placeholder="Functioneel ontwerp" value="${esc(values.description||'')}"><select name="deliverable_kind" title="Document: een Markdown-bestand. Visueel: een map met index.html (eigen CSS en JS), een afbeelding of een PDF, getoond in een sandbox"><option value="markdown" ${values.kind!=='visual'?'selected':''}>Document</option><option value="visual" ${values.kind==='visual'?'selected':''}>Visueel</option></select><label style="margin:0"><input name="deliverable_required" type="checkbox" style="width:auto" ${values.required===false?'':'checked'}> verplicht</label><button class="icon-button" type="button" title="Verwijder">${icon('delete')}</button>`;row.querySelector('button').onclick=()=>{row.remove();refreshTemplateInjections();};row.querySelector('[name=deliverable_name]').oninput=refreshTemplateInjections;container.appendChild(row);refreshTemplateInjections();}
