...
function handleACPEvent(event){
  ...
  if(event.type==='turn_end'){chatState.queued=event.queued||0;chatState.busy=chatState.queued>0;renderChatBusy();chatSystem(event.stop_reason?`Turn ended · ${event.stop_reason}`:'Turn ended');chatState.messageNodes.delete('agent:current');chatState.thoughtNodes.delete('thought:current');chatState.planNode=null;scheduleChatChanges(true);return;}
