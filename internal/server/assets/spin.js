...
// A version that an EDIT replaced stays in state for the layers built on it,
...
function workflowDecisionRoutes(question,template){if(question.kind==='action')return `<div class="decision-routes"><span class="decision-route accept">RETRY → ${esc(workflowTargetLabel(template,question.accept_target))}</span></div>`;return `<div class="decision-routes"><span class="decision-route accept">ACCEPT → ${finalAccept(question)?'Pull request of direct mergen':esc(workflowTargetLabel(template,question.accept_target))}</span><span class="decision-route reject">REJECT → ${esc(workflowTargetLabel(template,question.reject_target))}</span></div>`;}
...
// finalAccept says whether accepting this decision lands the Job: then the
// person chooses here how, a pull request or a direct merge.
function finalAccept(question){return question?.kind==='approval'&&question.accept_target==='spin-pull-request';}
function acceptButtons(question,cls,lock,job){if(!finalAccept(question))return `<button class="${cls}" data-accept-question="${esc(question?.id)}"${lock}>${icon('check')}ACCEPT</button>`;const base=job?.base_ref||'basisbranch';return `<button class="${cls}" data-accept-question="${esc(question.id)}" data-accept-finalize="pull_request"${lock} title="Job-branch als pull request op de remote">${icon('check')}ACCEPT · PULL REQUEST</button><button class="${cls}" data-accept-question="${esc(question.id)}" data-accept-finalize="merge"${lock} title="Spin merget de Job-branch direct in ${esc(base)}">${icon('merge')}ACCEPT · MERGEN IN ${esc(base.toUpperCase())}</button>`;}
function bindQuestionButtons(root=document){root.querySelectorAll('[data-accept-question]').forEach(button=>button.onclick=()=>decideQuestion(button.dataset.acceptQuestion,'accept','',button.dataset.acceptFinalize||''));root.querySelectorAll('[data-reject-question]').forEach(button=>button.onclick=()=>openRejectDecision(button.dataset.rejectQuestion));root.querySelectorAll('[data-answer-question]').forEach(button=>button.onclick=()=>answerQuestionForm(button.dataset.answerQuestion,button));
...
function renderArtifacts(){
  ...
  root.innerHTML=artifacts.map(artifact=>{
    ...
    // what the agent offered; the settings are metadata and survive an EDIT.
