...
function workflowDecisionTitle(question){if(question.kind==='agent')return 'AI VRAAGT INPUT';if(question.kind==='action')return 'PULL REQUEST MISLUKT';if(byID(snapshot.sessions,question.session_id)?.executor==='expose')return 'TEST-APP STAAT KLAAR';return question.outcome==='reject'?'AI REJECTED':'AI ACCEPTED';}
...
document.getElementById('git-form').onsubmit=async event=>{event.preventDefault();const form=new FormData(event.target),repositoryID=editingGitRepositoryID,payload={operator:currentOperator(),name:form.get('name'),remote_url:form.get('remote_url'),default_ref:form.get('default_ref'),credential_scope:form.get('credential_scope'),layer_selectors:selectedValues(document.getElementById('git-layers')),services:collectServices()};try{await api(repositoryID?`/api/git/repositories/${encodeURIComponent(repositoryID)}`:'/api/git/repositories',{method:repositoryID?'PUT':'POST',body:JSON.stringify(payload)});resetGitForm();closeDialog('git-dialog');await refresh(true);}catch(error){showError(error);}};
