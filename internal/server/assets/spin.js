...
function renderLogins(){
  ...
  document.getElementById('logins-copy').textContent=(logins.length?`${logins.length} login${logins.length===1?'':'s'}; ${logins.filter(login=>login.composition_id).length} in gebruik. De laag zelf is login 1.`:'Nog geen login: de eerste capsule maakt er één van wat de laag zelf bevat.')+(artifact.scope==='global'?' Gedeelde laag: deze logins zijn een pool voor iedereen.':'');
  document.getElementById('logins-body').innerHTML=logins.length?logins.map(login=>`<div class="login-row"><span class="material-symbols-outlined" aria-hidden="true">key</span><div class="login-main"><strong>Login ${login.number}</strong> ${login.owner?`<span class="tag" title="Alleen voor ${esc(login.owner)}">privé · ${esc(login.owner)}</span>`:artifact.scope==='global'?'<span class="tag">voor iedereen</span>':''} ${login.composition_id?`<span class="tag held">in gebruik · ${esc(loginHolderLabel(login.composition_id))}</span>`:'<span class="tag free">vrij</span>'}<small>${login.files} bestand${login.files===1?'':'en'} · ${esc(formatBytes(login.bytes||0))} · bijgewerkt ${esc(new Date(login.updated_at).toLocaleString())} · sinds ${esc(new Date(login.created_at).toLocaleDateString())}</small><div class="login-files" data-login-files-for="${esc(login.id)}" hidden></div></div><button class="small-button" data-login-files="${esc(login.id)}" title="Welke bestanden deze login bevat; wat een agent verzamelt kun je hier uitsluiten">${icon('folder_open')}Bestanden</button>${mayRemove?`<button class="danger" data-delete-login="${esc(login.id)}" title="${login.composition_id?'Verwijder deze login en sluit de capsule die hem vasthoudt':'Verwijder deze login; de capsules die erna starten krijgen een andere'}">${icon('delete')}Verwijder</button>`:''}</div>`).join(''):'<div class="empty">Nog geen logins.</div>';
