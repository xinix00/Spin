...
// A Job may span more repositories: rows below the main one, each with a
// mode (change on the Job branch, or read only) and its own base.
let jobExtraRepositories=[];
function repositoryOptionsHTML(selected){return snapshot.git_repositories.map(repository=>`<option value="${esc(repository.id)}" ${repository.id===selected?'selected':''}>${esc(repository.name)} · ${esc(repository.default_ref)}</option>`).join('');}
function renderJobRepositoryRows(){const root=document.getElementById('job-repositories'),primary=document.getElementById('job-git').value;root.innerHTML=jobExtraRepositories.map((row,index)=>{const repository=byID(snapshot.git_repositories,row.repository_id);return `<div class="job-repository-row"><select data-repo-row="${index}" data-repo-field="repository_id" aria-label="Repository">${repositoryOptionsHTML(row.repository_id)}</select><select data-repo-row="${index}" data-repo-field="mode" aria-label="Wat doet de Job ermee"><option value="change" ${row.mode!=='reference'?'selected':''}>Aanpassen</option><option value="reference" ${row.mode==='reference'?'selected':''}>Ter referentie</option></select><input data-repo-row="${index}" data-repo-field="base_ref" value="${esc(row.base_ref||'')}" placeholder="${esc(repository?.default_ref||'default branch')}" aria-label="Start vanaf"><button class="icon-button" type="button" data-repo-remove="${index}" aria-label="Weglaten" title="Deze repository weglaten">${icon('close')}</button>${repository&&repository.id===primary?`<span class="warning" style="grid-column:1/-1">Dit is al de hoofdrepository.</span>`:''}</div>`;}).join('');
  root.querySelectorAll('[data-repo-field]').forEach(field=>field.onchange=()=>{jobExtraRepositories[Number(field.dataset.repoRow)][field.dataset.repoField]=field.value;if(field.dataset.repoField==='repository_id')renderJobRepositoryRows();});
  root.querySelectorAll('[data-repo-remove]').forEach(button=>button.onclick=()=>{jobExtraRepositories.splice(Number(button.dataset.repoRemove),1);renderJobRepositoryRows();});}
document.getElementById('job-repository-add').onclick=()=>{const primary=document.getElementById('job-git').value,taken=new Set([primary,...jobExtraRepositories.map(row=>row.repository_id)]),free=snapshot.git_repositories.find(repository=>!taken.has(repository.id))||snapshot.git_repositories[0];if(!free){showError(new Error('Configureer eerst een tweede repository onder Connections → Git.'));return;}jobExtraRepositories.push({repository_id:free.id,mode:'reference',base_ref:''});renderJobRepositoryRows();};
function jobRepositoriesPayload(form){if(!jobExtraRepositories.length)return undefined;const primary={repository_id:form.get('git_repository_id'),mode:'change',base_ref:String(form.get('base_ref')||'').trim()};return [primary,...jobExtraRepositories.map(row=>({repository_id:row.repository_id,mode:row.mode||'change',base_ref:String(row.base_ref||'').trim()}))];}
