...
let snapshot = {artifacts:[],recordings:[],compositions:[],jobs:[],job_attachments:[],workflow_templates:[],phase_runs:[],deliverables:[],deliverable_comments:[],workflow_questions:[],sessions:[],activations:[],turns:[],checkpoints:[],results:[],clients:[],mcp_servers:[],git_repositories:[],git_accounts:[],git_oauth_providers:[],users:[],recommendations:[]};
...
let jobSubmitting = false, pendingJobSubmission = null, attachmentTargetJobID = '';
...
const jobChangesState = {jobID:'',sessionID:'',changes:{branch:'',added:0,deleted:0,files:[]},selectedPath:''};
let terminalLines = [
...
function handleACPEvent(event){
  if(event.type==='ready'){chatState.busy=!!event.busy;document.getElementById('chat-status').className=`chat-live ${chatState.busy?'busy':'ready'}`;document.getElementById('chat-status').innerHTML=`<span class="dot"></span><span>${esc(event.agent_name||'ACP')} · ${chatState.busy?'working':'ready'}</span>`;document.getElementById('chat-context').dataset.agentSession=event.agent_session_id||'';renderChatBusy();scheduleChatChanges(true);return;}
  if(event.type==='user'){chatMessage('user',event.text||'');chatState.messageNodes.delete('agent:current');chatState.thoughtNodes.delete('thought:current');chatState.busy=true;renderChatBusy();return;}
  ...
  if(event.type==='turn_end'){chatState.busy=false;renderChatBusy();chatSystem(event.stop_reason?`Turn ended · ${event.stop_reason}`:'Turn ended');chatState.messageNodes.delete('agent:current');chatState.thoughtNodes.delete('thought:current');chatState.planNode=null;scheduleChatChanges(true);return;}
  if(event.type==='error'){chatState.busy=false;if(event.fatal)chatState.manualClose=true;renderChatBusy();chatSystem(event.error||'ACP error',true);}
}
function renderChatBusy(){const ready=chatState.socket?.readyState===WebSocket.OPEN;document.getElementById('chat-send').disabled=!ready||chatState.busy;document.getElementById('chat-cancel').hidden=!chatState.busy;document.getElementById('chat-input').disabled=!ready;}
...
function renderChatQuestion(sessionID){const question=snapshot.workflow_questions.find(item=>item.session_id===sessionID&&item.status==='open');if(!question)return;const session=byID(snapshot.sessions,sessionID),job=byID(snapshot.jobs,session?.job_id),run=byID(snapshot.phase_runs,question.phase_run_id),template=job?jobTemplate(job):null,approvals=snapshot.workflow_questions.filter(item=>item.phase_run_id===run?.id&&item.kind==='approval'),node=document.createElement('section');node.className='question-card';node.dataset.chatQuestion=question.id;node.innerHTML=`<strong>${workflowDecisionTitle(question)}</strong>${question.kind==='approval'&&run?workflowDecisionHistory(run,approvals,job):`<p>${esc(question.question)}</p>`}${workflowDecisionRoutes(question,template)}<div class="panel-actions"><button class="primary" data-accept-question="${esc(question.id)}">ACCEPT</button><button class="danger" data-reject-question="${esc(question.id)}">REJECT</button></div>`;chatAppend(node);bindDetailStates(node);bindQuestionButtons(node);}
...
function bindQuestionButtons(root=document){root.querySelectorAll('[data-accept-question]').forEach(button=>button.onclick=()=>decideQuestion(button.dataset.acceptQuestion,'accept'));root.querySelectorAll('[data-reject-question]').forEach(button=>button.onclick=()=>openRejectDecision(button.dataset.rejectQuestion));}
...
function diffCell(line,side,language){
  if(!line)return '<div class="diff-cell diff-cell-empty"><span class="line-number"></span><code></code></div>';
  const number=side==='old'?line.old:line.new;return `<div class="diff-cell ${esc(line.type)}"><span class="line-number">${number??''}</span><code>${highlightCode(line.text,language)}</code></div>`;
}
function renderSplitDiff(hunks,language){
  ...
  hunks.forEach(hunk=>{rows.push(`<div class="diff-hunk">${esc(hunk.header)}</div>`);for(let index=0;index<hunk.lines.length;){const line=hunk.lines[index];
    if(line.type==='context'){rows.push(`<div class="diff-side-row">${diffCell(line,'old',language)}${diffCell(line,'new',language)}</div>`);index++;continue;}
    ...
    const count=Math.max(deletes.length,adds.length);for(let offset=0;offset<count;offset++)rows.push(`<div class="diff-side-row">${diffCell(deletes[offset],'old',language)}${diffCell(adds[offset],'new',language)}</div>`);
  }});return `<div class="diff-split">${rows.join('')}</div>`;
}
function renderUnifiedDiff(hunks,language){
  const rows=[];hunks.forEach(hunk=>{rows.push(`<div class="diff-hunk">${esc(hunk.header)}</div>`);hunk.lines.forEach(line=>{
    ...
    rows.push(`<div class="diff-row ${esc(line.type)}"><span class="line-number">${line.old??''}</span><span class="line-number">${line.new??''}</span><code><span class="diff-prefix">${prefix}</span>${highlightCode(line.text,language)}</code></div>`);
...
function renderDiff(file){
  ...
  return `${warnings.map(warning=>`<div class="diff-warning">${esc(warning)}</div>`).join('')}${hunks.length?renderSplitDiff(hunks,language)+renderUnifiedDiff(hunks,language):'<div class="diff-empty">Voor dit bestand is geen tekst-diff beschikbaar.</div>'}`;
}
...
async function openJobChanges(button){
  ...
  if(!job)return;jobChangesState.jobID=job.id;jobChangesState.sessionID=sessionID;jobChangesState.selectedPath='';jobChangesState.changes={branch:'',added:0,deleted:0,files:[]};
  document.getElementById('job-changes-title').textContent=sessionID?`${run?.phase_name||'Stap'} changes`:`${job.title} changes`;
  document.getElementById('job-changes-context').textContent=sessionID?`Alleen de wijzigingen van ${run?.phase_name||sessionID}.`:`Alles op ${job.branch} sinds ${job.base_ref}.`;
  document.getElementById('job-changes-added').textContent='+0';document.getElementById('job-changes-deleted').textContent='−0';document.getElementById('job-changes-file-count').textContent='0';document.getElementById('job-changes-branch').textContent='Git-vergelijking laden…';document.getElementById('job-changes-list').innerHTML='<div class="empty">Changes laden…</div>';document.getElementById('job-change-file-name').textContent='Selecteer een bestand';document.getElementById('job-change-file-meta').textContent='Side by side op brede schermen · inline op smalle schermen';document.getElementById('job-change-diff').innerHTML='<div class="diff-empty">Git-history en worktree worden vergeleken…</div>';openDialog('job-changes-dialog');
  const endpoint=live&&sessionID?`/api/sessions/${encodeURIComponent(sessionID)}/changes`:`/api/jobs/${encodeURIComponent(job.id)}/changes${sessionID?`?session_id=${encodeURIComponent(sessionID)}`:''}`;
  try{renderJobChanges(await api(endpoint));}catch(error){document.getElementById('job-changes-branch').textContent='Vergelijking mislukt';document.getElementById('job-changes-list').innerHTML=`<div class="empty">${esc(error.message||error)}</div>`;document.getElementById('job-change-diff').innerHTML='<div class="diff-empty">De opgeslagen Git-workspace kon niet worden vergeleken.</div>';}
}
...
function renderJobs(){
  ...
  root.innerHTML=jobs.map(job=>{
    ...
    const runHTML=runs.map(run=>{
      ...
      const questionButtons=question?.kind==='action'?`<button class="primary" data-accept-question="${esc(question.id)}">${icon('replay')}RETRY PR</button>`:`<button class="primary" data-accept-question="${esc(question?.id)}">${icon('check')}ACCEPT</button><button class="danger" data-reject-question="${esc(question?.id)}">${icon('cancel')}REJECT</button>${active&&ready&&acp?`<button class="small-button" data-open-acp="${esc(session.id)}">${icon('chat')}CHAT</button>`:''}`;
      const questionHTML=question?`<div class="question-card" data-workflow-question="${esc(question.id)}"><strong>${workflowDecisionTitle(question)}</strong>${question.kind==='approval'?decisionHTML:`<p>${esc(question.question)}</p>`}${workflowDecisionRoutes(question,template)}<div class="panel-actions">${questionButtons}</div></div>`:'';
...
function renderRunners(){
  ...
  root.innerHTML=snapshot.clients.map(client=>{
    ...
    const statusClass=client.status==='online'?'capability':(client.status==='draining'?'warning':'');
    return `<article class="mcp-card"><div><div class="artifact-title"><strong>${esc(client.name)}</strong><span class="tag ${statusClass}">${esc(client.status||'offline')}</span><span class="tag">${esc(engine.driver||'unknown engine')}</span></div><small>${esc(client.capabilities?.os||'?')}/${esc(client.capabilities?.arch||'?')} · ${esc(client.id)} · max ${esc(client.capabilities?.max_workloads||'∞')} workloads</small><small>${active.length} actief · ${pinned.length} Sessions gekoppeld · laatst gezien ${esc(formatDateTime(client.last_seen_at))}</small></div></article>`;
  }).join('');
}
...
function renderAccess(){
  ...
  root.innerHTML=snapshot.users.length?snapshot.users.map(user=>`<article class="mcp-card"><div><div class="artifact-title"><strong>${esc(user.display_name||user.username)}</strong><span class="tag ${user.role==='admin'?'secret':''}">${esc(user.role)}</span>${user.id===authState.user?.id?'<span class="tag capability">you</span>':''}</div><small>@${esc(user.username)} · created ${esc(new Date(user.created_at).toLocaleDateString())}</small></div></article>`).join(''):'<div class="empty">Nog geen gebruikers.</div>';
}
...
async function refresh(force=false){
  try{
    ...
    const engine=snapshot.engine||{},onlineClients=snapshot.clients.filter(client=>client.status==='online').length,totalClients=snapshot.clients.length,clientLabel=`${onlineClients}/${totalClients} client${totalClients===1?'':'s'} connected`;
...
document.getElementById('open-job-dialog').onclick=()=>{setJobStep(1);openDialog('job-dialog');};
...
document.getElementById('chat-form').onsubmit=event=>{event.preventDefault();const input=document.getElementById('chat-input'),text=input.value.trim();if(!text||chatState.busy)return;sendChat({type:'prompt',text});document.querySelectorAll('[data-chat-question]').forEach(node=>node.remove());input.value='';input.style.height='auto';setTimeout(()=>refresh(),250);};
