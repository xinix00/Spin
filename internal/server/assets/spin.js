...
function retrySession(button){const id=button.dataset.retrySession;if(!id)return;pendingRetry={id,button,fromChat:button.id==='chat-retry'};document.getElementById('retry-form').reset();openDialog('retry-dialog');requestAnimationFrame(()=>document.getElementById('retry-note').focus());}
document.getElementById('retry-form').onsubmit=async event=>{event.preventDefault();if(!pendingRetry)return;const {id,button,fromChat}=pendingRetry,note=String(new FormData(event.target).get('note')||'').trim();pendingRetry=null;closeDialog('retry-dialog');const label=button.innerHTML;button.disabled=true;button.textContent='Opnieuw starten…';try{await api(`/api/sessions/${encodeURIComponent(id)}/retry`,{method:'POST',body:JSON.stringify({note})});if(fromChat)closeDialog('chat-dialog');showNotice('Stap opnieuw gestart met een verse agent; de workspace is bewaard.');await refresh(true);}catch(error){button.disabled=false;button.innerHTML=label;showError(error);}};
