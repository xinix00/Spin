...
function connectACPChat(sessionID){
  ...
  if(!chatSessionReady(sessionID)){const session=byID(snapshot.sessions,sessionID),status=document.getElementById('chat-status');status.className='chat-live busy';
    ...
    const closed=byID(snapshot.compositions,session?.prepared_composition_id);if(closed?.runtime?.status==='stopped'&&chatState.restartAsked!==sessionID){chatState.restartAsked=sessionID;chatSystem('De capsule van deze stap was gesloten; hij wordt opnieuw gestart.');api(`/api/sessions/${encodeURIComponent(sessionID)}/capsule`,{method:'POST'}).catch(error=>chatSystem(error.message||String(error),true));}status.innerHTML=`<span class="dot"></span><span>${esc(chatWaitingLabel(session))}</span>`;if(!chatState.waitingShown){chatState.waitingShown=true;chatSystem('De omgeving van deze Session wordt klaargezet. Zodra de agent er is, kun je typen.');}clearTimeout(chatState.reconnectTimer);chatState.reconnectTimer=setTimeout(()=>{if(document.getElementById('chat-dialog').open&&chatState.sessionID===sessionID)connectACPChat(sessionID);},1500);return;}
