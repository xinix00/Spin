...
function renderJobs(){
  ...
  root.innerHTML=jobs.map(job=>{
    ...
    const runHTML=runs.map(run=>{
      ...
      const answerButton=question?.kind==='agent'&&question.items?.length?`<button class="primary" data-answer-question="${esc(question.id)}">${icon('send')}BEANTWOORD</button>`:'';
      const questionButtons=question?.kind==='action'?`<button class="primary" data-accept-question="${esc(question.id)}">${icon('replay')}RETRY PR</button>`:`${answerButton}<button class="${answerButton?'small-button':'primary'}" data-accept-question="${esc(question?.id)}">${icon('check')}ACCEPT</button><button class="${answerButton?'small-button':'danger'}" data-reject-question="${esc(question?.id)}">${icon('cancel')}REJECT</button>${active&&ready&&acp?`<button class="small-button" data-open-acp="${esc(session.id)}">${icon('chat')}CHAT</button>`:''}`;
      const questionHTML=question?`<div class="question-card" data-workflow-question="${esc(question.id)}"><strong>${workflowDecisionTitle(question)}</strong>${question.kind==='approval'?decisionHTML:agentQuestionFormHTML(question)}${workflowDecisionRoutes(question,template)}<div class="panel-actions">${questionButtons}</div></div>`:'';
