...
function loginsButton(artifact){if(artifact.kind!=='credential'||!(artifact.tracked_paths||[]).length)return '';const logins=loginsOf(artifact),held=logins.filter(login=>login.composition_id).length;return `<button class="small-button" data-logins-artifact="${esc(artifact.id)}" title="Logins van deze laag: elke draaiende capsule krijgt er één voor zich alleen">${icon('key')}Logins${logins.length?` · ${logins.length}${held?` (${held} in gebruik)`:''}`:''}</button>`;}
...
function renderLogins(){
  ...
  const logins=loginsOf(artifact),mayRemove=artifact.subject===currentOperator()||artifact.created_by===currentOperator()||authState.user?.role==='admin';
