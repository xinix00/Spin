...
function renderContentsNode(node,depth,prefix,track,openAll){
  ...
  return dirs.map(([name,child])=>{const path=prefix?`${prefix}/${name}`:name,open=openAll||contentsOpenFolders.has(path)||(depth<1&&!contentsOpenFolders.has('!'+path)),below=contentsFilesBelow(child),bytes=below.reduce((sum,file)=>sum+(file.size||0),0),all=below.length>0&&below.every(file=>contentsState.tracked.has(file.path)),some=below.some(file=>contentsState.tracked.has(file.path));
    return `<div class="tree-folder${open?' open':''}"><div class="tree-row">${track?`<input type="checkbox" data-track-folder="${esc(path)}" ${all?'checked':''} ${!all&&some?'data-indeterminate="1"':''} title="Alles in deze map bijhouden">`:''}<button type="button" class="tree-row-toggle" data-toggle-contents-folder="${esc(path)}" title="${esc('/'+path)}"><span class="material-symbols-outlined tree-slot" aria-hidden="true">chevron_right</span><span class="material-symbols-outlined tree-icon folder" aria-hidden="true">${open?'folder_open':'folder'}</span><span class="tree-name">${esc(name)}</span></button><span class="tree-size">${below.length} · ${esc(formatBytes(bytes))}</span></div><div class="tree-children" ${open?'':'hidden'}>${renderContentsNode(child,depth+1,path,track,openAll)}</div></div>`;}).join('')+
    files.map(file=>`<div class="tree-row tree-file">${track?`<input type="checkbox" data-track-path="${esc(file.path)}" ${contentsState.tracked.has(file.path)?'checked':''}>`:''}<span class="tree-slot"></span><span class="material-symbols-outlined tree-icon" aria-hidden="true">${fileIcon(file.name)}</span><span class="tree-name" title="${esc(file.path)}">${esc(file.name)}</span><span class="tag">${esc(kindLabel(file.kind))}</span><span class="tree-size">${esc(formatBytes(file.size||0))}</span></div>`).join('');
}
function renderContentsList(){
  ...
  const track=Boolean(contentsState.artifactID);
  root.innerHTML=rows.length?`<div class="code-tree contents-tree">${renderContentsNode(contentsTreeModel(rows),0,'',track,Boolean(filter||kind))}</div>`:'<div class="empty">Niets gevonden.</div>';
  root.querySelectorAll('[data-indeterminate]').forEach(box=>box.indeterminate=true);
  root.querySelectorAll('[data-toggle-contents-folder]').forEach(button=>button.onclick=()=>{const path=button.dataset.toggleContentsFolder,folder=button.closest('.tree-folder'),open=!folder.classList.contains('open');folder.classList.toggle('open',open);folder.querySelector(':scope>.tree-children').hidden=!open;button.querySelector('.tree-icon').textContent=open?'folder_open':'folder';if(open){contentsOpenFolders.add(path);contentsOpenFolders.delete('!'+path);}else{contentsOpenFolders.delete(path);contentsOpenFolders.add('!'+path);}});
  root.querySelectorAll('[data-track-path]').forEach(box=>box.onchange=()=>{if(box.checked)contentsState.tracked.add(box.dataset.trackPath);else contentsState.tracked.delete(box.dataset.trackPath);renderContentsList();});
...
function addTemplateStep(values={}){
  ...
  step.querySelector('[name=accept_target]').value=isDone(values.accept?.target)?doneValue(values.accept):values.accept?.target||'NEXT';step.querySelector('[name=reject_target]').value=isDone(values.reject?.target)?doneValue(values.reject):values.reject?.target||previous;
