...
function renderArtifacts(){
  ...
  root.innerHTML=artifacts.map(artifact=>{
    ...
    const fetchOptions=acp?`<button class="small-button" data-fetch-options="${esc(artifact.id)}" title="Start de agent één keer in een wegwerp-capsule (deze laag plus de lagen erboven, zoals je credential-laag) en bewaar welke modes, modellen en reasoning-niveaus hij aanbiedt">${icon('tune')}${options&&!options.error?'Opties vernieuwen':'Opties ophalen'}</button>`:'';
