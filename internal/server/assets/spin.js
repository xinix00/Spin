...
function fullFileURL(file,sessionID){const path=fileInRepository(file);if(file.head&&file.repository)return `/api/git/repositories/${encodeURIComponent(file.repository)}/code/file?${new URLSearchParams({ref:file.head,path})}`;if(sessionID)return `/api/sessions/${encodeURIComponent(sessionID)}/file?${new URLSearchParams({path,folder:file.folder||''})}`;return '';}
