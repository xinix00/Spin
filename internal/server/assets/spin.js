...
function contentsButton(artifact){if(!artifact.snapshot?.restorable)return '';return artifact.snapshot?.contents?`<button class="small-button" data-contents-artifact="${esc(artifact.id)}" title="Alle paden van deze laag, per soort">${icon('inventory_2')}Inhoud</button>`:`<button class="small-button" data-inspect-artifact="${esc(artifact.id)}" title="Deze laag is opgeslagen voordat Spin inhoudsopgaven bijhield; een runner leest het verschil uit de image">${icon('manage_search')}Inhoud bepalen</button>`;}
// The contents dialog lists every path of a layer's manifest or a capsule's
// changes, largest first, filtered by text and kind.
const contentsState={entries:[],title:''};
async function openContents(url,title){
  try{
    const result=await api(url),contents=result.contents||{},entries=result.entries||[];
    contentsState.entries=entries;contentsState.title=title;
...
function renderContentsList(){
  ...
  const rows=contentsState.entries.filter(entry=>(!kind||entry.kind===kind)&&(!filter||entry.path.toLowerCase().includes(filter))).slice(0,2000);
  root.innerHTML=rows.length?`<table class="contents-table"><thead><tr><th>Pad</th><th>Soort</th><th>Grootte</th></tr></thead><tbody>${rows.map(entry=>`<tr><td><code>${esc(entry.path)}</code></td><td><span class="tag">${esc(kindLabel(entry.kind))}</span></td><td>${esc(formatBytes(entry.bytes))}</td></tr>`).join('')}</tbody></table>${contentsState.entries.length>rows.length?`<p class="hint">Eerste ${rows.length} van ${contentsState.entries.length}; verfijn het filter.</p>`:''}`:'<div class="empty">Niets gevonden.</div>';
}
...
function renderArtifacts(){
  ...
  root.querySelectorAll('[data-inspect-artifact]').forEach(button=>button.onclick=async()=>{button.disabled=true;button.innerHTML=`${icon('hourglass_top')}Lezen…`;try{await api(`/api/artifacts/${encodeURIComponent(button.dataset.inspectArtifact)}/contents/inspect`,{method:'POST'});await refresh(true);}catch(error){showError(error);await refresh(true);}});
  root.querySelectorAll('[data-contents-artifact]').forEach(button=>button.onclick=()=>{const artifact=byID(snapshot.artifacts,button.dataset.contentsArtifact);openContents(`/api/artifacts/${encodeURIComponent(button.dataset.contentsArtifact)}/contents`,`Inhoud van ${artifact?artifactSelector(artifact):'laag'}`);});
