...
function renderLogins(){
  ...
  const logins=loginsOf(artifact),mayRemove=artifact.subject===currentOperator()||authState.user?.role==='admin';
  document.getElementById('logins-title').textContent=`Logins van ${artifactSelector(artifact)}`;
  document.getElementById('logins-copy').textContent=logins.length?`${logins.length} login${logins.length===1?'':'s'}; ${logins.filter(login=>login.composition_id).length} in gebruik. De laag zelf is login 1.`:'Nog geen login: de eerste capsule maakt er één van wat de laag zelf bevat.';
