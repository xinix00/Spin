...
function renderDeliverableRevision(id){
  ...
  const download=document.getElementById('deliverable-download'),fileName=`${deliverableFileName(item.name)}-r${item.revision}.md`;download.href=`/api/deliverables/${encodeURIComponent(item.id)}/download`;download.download=fileName;document.getElementById('deliverable-download-name').textContent=fileName;
...
function openAdoptTemplate(jobID){const job=byID(snapshot.jobs,jobID),latest=job&&byID(snapshot.workflow_templates,job.template_id);if(!job||!latest)return;adoptTemplateJobID=jobID;const current=jobTemplate(job),run=byID(snapshot.phase_runs,job.current_phase_run_id),currentExists=run&&(latest.phases||[]).some(phase=>phase.id===run.phase_id);
  ...
  const select=document.getElementById('adopt-template-phase');select.innerHTML=`${currentExists?`<option value="">Huidige stap behouden (${esc(run.phase_name)})</option>`:''}${(latest.phases||[]).filter(phase=>phase.id!=='spin-pull-request').map((phase,index)=>`<option value="${esc(phase.id)}">Verder bij ${index+1}. ${esc(phase.name)}</option>`).join('')}`;
  openDialog('adopt-template-dialog');}
document.getElementById('adopt-template-form').onsubmit=async event=>{event.preventDefault();if(!adoptTemplateJobID)return;const phase=document.getElementById('adopt-template-phase').value;try{await api(`/api/jobs/${encodeURIComponent(adoptTemplateJobID)}/template`,{method:'POST',body:JSON.stringify({phase_id:phase})});closeDialog('adopt-template-dialog');showNotice(phase?'Job gaat verder op de nieuwe Template-revisie':'Job draait nu op de nieuwe Template-revisie');await refresh(true);}catch(error){showError(error);}};
...
// A merge step's DONE is the end of the Job: the merge was the landing.
function templateTargetOptions(selected='',includeSelf=true,mergeStep=false){const steps=[...document.querySelectorAll('[data-template-step]')],options=[['NEXT','Volgende stap']];if(includeSelf)options.push(['SELF','Dezelfde stap']);steps.forEach((step,index)=>options.push([step.dataset.stepId,`${index+1}. ${step.querySelector('[name=phase_name]').value||'Naamloze stap'}`]));if(mergeStep)options.push(['DONE','Job afronden · klaar']);else options.push(['DONE:pull_request','Job afronden · pull request maken'],['DONE:merge','Job afronden · direct mergen in de basisbranch']);return options.map(([value,label])=>`<option value="${esc(value)}" ${value===selected?'selected':''}>${esc(label)}</option>`).join('');}
function refreshTemplateTargets(){document.querySelectorAll('[data-transition-target]').forEach(select=>{const current=select.value,includeSelf=select.dataset.transitionTarget==='reject',mergeStep=select.closest('[data-template-step]')?.querySelector('[name=phase_executor]')?.value==='action';select.innerHTML=templateTargetOptions(current,includeSelf,mergeStep);if([...select.options].some(option=>option.value===current))select.value=current;else if(mergeStep&&String(current).startsWith('DONE'))select.value='DONE';});}
...
function addTemplateStep(values={}){
  ...
  const doneValue=transition=>`DONE:${transition?.landing||document.getElementById('template-finalize')?.value||'pull_request'}`,isDone=target=>target==='spin-pull-request'||target==='DONE'||String(target||'').startsWith('DONE:');
  const merge=values.executor==='action',doneFor=transition=>merge?'DONE':doneValue(transition);
  step.querySelector('[name=accept_target]').value=isDone(values.accept?.target)?doneFor(values.accept):values.accept?.target||(merge?'DONE':'NEXT');step.querySelector('[name=reject_target]').value=isDone(values.reject?.target)?doneFor(values.reject):values.reject?.target||previous;
