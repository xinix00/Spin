...
function sessionPresenceHTML(session){if(!session)return '';if(!session.client_id)return `<small class="session-presence">${session.status==='queued'?'Wacht op runner':'Nog geen runner gekoppeld'}</small>`;const client=byID(snapshot.clients,session.client_id);if(client&&['online','draining'].includes(client.status))return `<small class="session-presence">Runner ${esc(client.name)} · ${esc(client.draining?'draining':'online')}</small>`;const name=client?.name||session.client_id,lastSeen=client?.last_seen_at;return `<small class="session-presence offline">${icon('cloud_off')} Geen actieve client · ${esc(name)}${lastSeen?` · ${esc(elapsedSince(lastSeen))} offline`:''}</small>`;}
...
function renderJobs(){
  ...
  root.innerHTML=jobs.map(job=>{
    ...
    const runHTML=runs.map(run=>{
      ...
      return `<details class="workflow-step ${stepIsRunning?'active':''}" data-detail-state="${esc(stepDetailKey)}" ${detailOpenAttribute(stepDetailKey,stepIsRunning)}><summary><span class="workflow-marker ${esc(run.status)}"></span><span class="workflow-step-copy"><strong>${esc(run.phase_name)} <span class="tag">poging ${run.attempt}</span>${session?.executor==='action'?'<span class="tag capability">ACTION</span>':''}</strong><small>${session?short(session.id):'geen Session'}</small>${active?presenceHTML:''}</span><span class="workflow-step-status">${esc(run.status)}</span></summary><div class="workflow-step-body"><div class="workflow-step-content">${outputs.length?`<div class="deliverable-chips">${outputs.map(item=>`<button class="deliverable-chip" data-deliverable="${esc(item.id)}">${esc(item.name)} · r${item.revision}</button>`).join('')}</div>`:''}${actionHTML}${actionFailure}${questionHTML}${question?.kind==='approval'?'':decisionHTML}</div>${actions?`<div class="session-actions">${actions}</div>`:''}</div></details>`;
    }).join('');
