...
function saveLogin(compositionID){return runAction(()=>api(`/api/compositions/${encodeURIComponent(compositionID)}/login`,{method:'POST'}),logins=>showNotice(logins.map(login=>`Login ${login.number} van ${login.key.split('/').slice(1).join('/')} bewaard`).join('; ')));}
