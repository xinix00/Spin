...
async function openContents(url,title,artifact=null){
  try{
    ...
    contentsState.tracked=new Set(artifact?(artifact.tracked_paths&&artifact.tracked_paths.length?artifact.tracked_paths:entries.filter(entry=>entry.kind==='login').map(entry=>entry.path)):[]);
...
function renderContentsList(){
  ...
  root.innerHTML=rows.length?`<table class="contents-table"><thead><tr>${track?'<th title="Bijhouden tussen Sessions: na elke beurt teruglezen, bij de volgende start terugzetten">Bijhouden</th>':''}<th>Pad</th><th>Soort</th><th>Grootte</th></tr></thead><tbody>${rows.map(entry=>`<tr>${track?`<td><input type="checkbox" data-track-path="${esc(entry.path)}" ${contentsState.tracked.has(entry.path)?'checked':''}></td>`:''}<td><code>${esc(entry.path)}</code></td><td><span class="tag">${esc(kindLabel(entry.kind))}</span></td><td>${esc(formatBytes(entry.bytes))}</td></tr>`).join('')}</tbody></table>${contentsState.entries.length>rows.length?`<p class="hint">Eerste ${rows.length} van ${contentsState.entries.length}; verfijn het filter.</p>`:''}`:'<div class="empty">Niets gevonden.</div>';
...
function artifactDescendants(id){const out=[];const walk=parent=>{snapshot.artifacts.forEach(candidate=>{if((candidate.parent_artifact_ids||[]).includes(parent)&&!out.includes(candidate)){out.push(candidate);walk(candidate.id);}});};walk(id);return out;}
