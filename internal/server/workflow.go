...
func (s *Server) mergeToResolve(sessionID string) string {
	...
	if err != nil || !phase.ResolveMerge {
		return ""
	}
	return strings.TrimSpace(job.BaseRef)
}
...
func (s *Server) workflowPromptWithOptions(sessionID string, attachInjectedDeliverables bool) (string, error) {
	...
	// A step of the kind "AI merge" gets the recipe with it; no other step
	// is touched.
	if phase.ResolveMerge {
		...
		fmt.Fprintf(&prompt, "\nMERGE OPLOSSEN\nDeze stap bestaat om %s in de Job-branch te krijgen:\n", base)
		...
		prompt.WriteString("3. `git add` de opgeloste bestanden en rond af met `git commit` zonder de tekst te veranderen. Dit is de enige stap waar je zelf commit; pushen doe je nooit.\n")
		prompt.WriteString("4. Bouw de merge nooit met de hand na: geen bestanden overschrijven, geen diff toepassen, geen nieuwe branch. Zonder echte merge-commit mislukt de Merge-stap opnieuw.\n")
		prompt.WriteString("5. Sluit af met accept en schrijf in één zin welke bestanden conflicteerden en hoe je ze hebt opgelost.\n")
		prompt.WriteString("Weigert git met \"refusing to merge unrelated histories\" of vindt hij geen merge-base, reject dan met precies die melding: de workspace is dan verkeerd klaargezet en Spin lost dat op, niet jij.\n")
	}
	if run.Restarts > 0 {
