...
func (s *Server) createCapsuleRecording(req domain.CreateRecordingRequest) (domain.Recording, *domain.StartStatus, error) {
	...
	now := time.Now().UTC()
	job := &startJob{done: make(chan struct{}), status: domain.StartStatus{
	...
	s.startMu.Unlock()
	go s.runStart(job, recording, parents)
...
func (s *Server) runStart(job *startJob, recording domain.Recording, parents []domain.Artifact) {
	// Detached from the request on purpose: the request may be long gone.
	ctx := capsule.WithProgress(context.Background(), func(stage, message string, current, total int64) {
