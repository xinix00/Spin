...
// missed page. On disk: magic, page size, page count, the hashes, and a
// checksum over all of it.

const indexMagic = "SPINIDX1"
...
func decodeIndex(data []byte) (pageIndex, error) {
	...
	count := int(binary.BigEndian.Uint32(body[len(indexMagic)+4:]))
	if pageSize < 512 || pageSize > 65536 || pageSize&(pageSize-1) != 0 || len(body) != head+count*len(pageHash{}) {
		return pageIndex{}, errors.New("page index is malformed")
	}
	x := pageIndex{pageSize: pageSize, hashes: make([]pageHash, count)}
...
func (r *Replica) writeIndex() error {
	...
	r.indexMu.Unlock()
	return r.writeLocal(r.indexPath(), data)
}
...
func (r *Replica) rebuildIndex(pageSize int) error {
	hashes, err := r.hashDatabase(pageSize)
	if err != nil {
		return err
	}
	r.indexMu.Lock()
	r.index = pageIndex{pageSize: pageSize, hashes: hashes}
	r.indexMu.Unlock()
	return r.writeIndex()
}
