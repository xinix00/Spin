...
- `put_deliverable(name, path)` zet een bestand of map uit `/root/deliverables` als nieuwe revisie van een deliverable. De deliverables van een Job staan in elke capsule van de Job als bestanden in die map: een document als `<slug>.md`, een visuele deliverable als map (met `index.html`, eigen CSS, JS en afbeeldingen mogen los) of als los bestand (afbeelding, PDF). De agent leest wat hij nodig heeft, bewerkt op schijf met zijn eigen gereedschap en put het resultaat; wat hij elders bouwt (`/tmp`, de workspace) verplaatst hij eerst naar `/root/deliverables`, want een pad daarbuiten wordt geweigerd. Deze tool bestaat alleen in een fase die deliverables vraagt. Eén Session levert precies één revisie: de eerste put maakt haar, iedere latere put in dezelfde Session werkt diezelfde revisie bij; een Session die niets put laat de vorige revisie staan. Een document is in de browser te downloaden als `<naam>-r<revisie>.md`, een visuele revisie als zip;
...
De Job Goal wordt altijd geïnjecteerd. De deliverables zelf staan niet in de prompt: de prompt noemt per deliverable het pad in `/root/deliverables`, de revisie en de vorm, en welke daarvan het Template voor deze fase als verplichte context aanvinkt. Zonder bestaande revisie van zo'n verplichte deliverable start de overgang niet.

**Soorten deliverables.** Bij een deliverable in het Template kies je de soort: **Document** (een Markdown-bestand), **PDF** (één .pdf), **Afbeelding** (één png, jpg, gif, webp of svg), **Map** (een map met minstens één bestand; met `index.html` erin toont Spin de pagina, anders de bestandenlijst) of **Bestand** (één bestand, welke vorm ook). De soort is de maat: `put_deliverable` weigert wat er niet aan voldoet, dus een opgeleverde deliverable is altijd wat het Template vroeg. Alles behalve een Document wordt door de runner als zip uit de capsule gehaald (ten hoogste 25 MB) en in de database bewaard; de reviewer ziet hem in de viewer in een iframe en kan hem in een eigen tab openen, met revisies naast elkaar en commentaar op de hele revisie. Een **Merge**-stap draait helemaal zonder capsule: mergen vraagt de branches en git op een runner, niet de agentlaag of een login van de Job, dus een teruggetrokken credential-laag houdt een merge niet meer tegen. Ook **accepteren** kan zonder capsule: een stap die op je antwoord wachtte heeft zijn capsule vrijgegeven, maar zijn werk staat op zijn Session-branch, dus vouwt de runner die op zijn eigen kloon als één commit op de Job-branch. De changes van een Job worden vergeleken op de eigen kloon van de runner, niet in een capsule van de Job: de branches staan op de remote en blijven daar, dus ook een Job waarvan de capsules allang opgeruimd zijn laat zijn changes en die van elke afgeronde stap gewoon zien. Elke geserveerde pagina krijgt een Content-Security-Policy met `sandbox allow-scripts`, `connect-src 'none'` en `form-action 'none'`: een eigen lege origin, geen toegang tot Spin, geen netwerk, wel werkende scripts en formulieren binnen de preview. De preview wordt geserveerd op een onraadbaar pad (`/preview/<token>/`, een kwartier geldig en vanzelf vernieuwd zolang je kijkt) in plaats van op het revisie-id: een sandbox heeft geen site voor cookies, dus de vervolgverzoeken van de pagina (stylesheet, scripts, afbeeldingen) komen zonder sessiecookie binnen en zouden op een ingelogd pad 401 opleveren. De eigen bestanden van de bundel staan in die policy ook op het adres waarop Spin bereikt werd en niet op `'self'`, want `'self'` matcht een lege origin nooit. Styling, fonts en afbeeldingen van een https-CDN mogen laden zodat een pagina eruitziet zoals hij bedoeld is; code komt alleen uit de bundel zelf. Met **Delen** krijgt een revisie een link `/share/<token>/` die iedereen een uur lang zonder inloggen kan openen (een Document als leesbare pagina); opnieuw delen houdt dezelfde link en verlengt hem, en het schakeltje sluit hem meteen. Zo kan een ontwerpstap een printformulier of een shop-styling laten zien met de echte CSS en JS, en een ontwikkelstap een screenshot of PDF.
...
Wat een laag geschreven heeft, is van die laag; Spin deelt paden niet in soorten in en kiest niets voor je. Bij End & save houdt een laag alleen zijn echte verschil over: Docker rekent een bestand als gewijzigd zodra het voor schrijven is geopend, dus een tool die zichzelf ongewijzigd terugschreef of een login die een hele installatie aanraakte, maakte een laag van honderden megabytes die niets toevoegde. Spin leest de diff van de laag, vergelijkt elk bestand met de laag eronder, laat weg wat byte voor byte gelijk is, en bouwt de laag opnieuw uit de rest als er iets wegviel. Verwijderen van een laag neemt alles mee wat erop gebouwd is, ook de credential-lagen van andere gebruikers; de bevestiging noemt ze. Draait er nog een capsule op zo'n laag of staat er een opname open, dan stopt Spin die eerst. De opnemer van een laag mag hem verwijderen, en een admin mag elke laag verwijderen. De laagkaart toont de inhoudsopgave: aantal bestanden, grootte, en wat is weggelaten; de knop Inhoud opent de volledige lijst als boom, met filter op pad, zodat je ziet wat er mis ging als een laag niet bevat wat je bedoelde. Die lijst staat als bestand naast de state, niet erin. Na elke beurt en bij het stoppen noteert Spin ook wat een capsule buiten de workspace veranderde, op de compositiekaart, met Bekijk voor de lijst.

Een agent ververst zijn OAuth-token terwijl hij werkt, en zo'n refresh-token is eenmalig. Bleef dat in de capsule achter, dan startte de volgende Session met het oude token uit de laag en weigerde de provider ("another process is refreshing it"). Daarom kan een laag bestanden en mappen **bijhouden**: in het scherm Inhoud vink je aan wat Spin tussen Sessions bewaart (een login, een config die de agent bijwerkt), op elke laag; Spin kiest niets voor je. Een aangevinkte map gaat helemaal mee, ook bestanden die er later bij komen (een tool dat naast zijn token nog een refresh-bestand neerzet); binnen zo'n map vink je uit wat niet mee hoeft, zoals caches en geschiedenis, en `*.lock`-bestanden gaan nooit mee. Een map wordt als geheel bewaard: een bestand dat de agent eruit verwijdert, verdwijnt ook uit de login. Per map komen hoogstens 2000 bestanden van elk maximaal 1 MiB mee. Die bestanden vormen een **login** van de laag, versleuteld in de database; de keuze gaat mee naar een nieuwe versie van de laag. Een credential-laag is een uitgeefsysteem: twee capsules die tegelijk hetzelfde token verversen maken elkaars login ongeldig, dus elke draaiende capsule krijgt bij de start één login voor zich alleen en geeft hem bij het stoppen terug. De laag zelf is login 1; wil je meer capsules tegelijk (twee Jobs, een Job en jouw eigen Session), dan druk je op de laagkaart op Logins → Nieuwe login: Spin start een capsule op de laag zonder login, je logt in via de shell (`claude login`, `codex login --device-auth`) en drukt op Bewaar login. Zo zijn het echt drie aparte aanmeldingen, zoals drie machines. De pool deelt om de beurt uit: een capsule krijgt de vrije login die het langst niet gebruikt is (eigen privé-logins gaan voor). Een login die door zijn tokens heen is zet je **uit**: hij gaat naar geen enkele capsule meer, en een capsule die hem vasthoudt stapt meteen over op een andere vrije login (de bestanden worden in de capsule vervangen, de taak loopt gewoon door). Is er geen andere login, dan sluit die capsule en wacht de stap tot er één vrijkomt. Aanzetten zet hem terug in de pool. Omdat de login niet meer in de laag zelf zit, kan een credential-laag ook **global** zijn: één gedeelde laag per agent, met daarop centraal ingesteld welke mappen en bestanden worden bijgehouden, en een pool van logins die aan elke draaiende capsule van wie dan ook wordt uitgedeeld. Een Job neemt de eigen credential-laag van de gebruiker als die er is, anders de gedeelde. In een gedeelde laag kan een login ook **privé** zijn (alleen voor mij bij Nieuwe login): die gaat alleen naar capsules van die gebruiker, en gaat vóór een gedeelde login. Het scherm Inhoud van een laag toont niet alleen de laag maar ook alles wat in haar logins staat, gemarkeerd met het loginnummer, zodat wie de laag beheert ziet welke mappen er bij anderen bij verschijnen en die kan uitsluiten.  Zijn alle logins in gebruik, dan wacht een Job tot er een vrijkomt en zegt dat erbij; een handmatige start meldt het meteen. Terwijl een capsule draait kijkt de runner naar de bijgehouden mappen en bestanden (inotify als de image `inotifywait` heeft, anders elke seconde een handtekening) en stuurt bij een wijziging de inhoud meteen naar de server, die hem in de vastgehouden login bewaart; een crash later kan dus niets meer kwijtraken. Bij het stoppen leest Spin nog één keer terug. Er reist niets tussen draaiende capsules. Bijgehouden bestanden op een andere laag dan een credential-laag (een config die voor iedereen gelijk is) hebben één login die elke capsule deelt. Alles wat de agent verder in de capsule deed, verdwijnt met de capsule.
...
Een Job kan in **meer repositories** tegelijk werken (iets voor de shop én de backoffice). In het Job-formulier is Repositories één lijst: de eerste regel staat er al, met + voeg je er een toe, en per regel zeg je wat de Job ermee doet: **Aanpassen** of **Ter referentie**, en vanaf welke branch. Met één repository staat die zoals altijd op `/workspace`; met meer krijgt elke repository zijn eigen map onder `/workspace` (`/workspace/shop`, `/workspace/backoffice`), en de prompt noemt ze. Elke aanpasbare repository krijgt dezelfde Job-branch en Session-branch, wordt na elke beurt gesynct, bij accept per repository als één commit op zijn Job-branch gezet, en de Merge- en Pull request-stappen doen hun werk per repository, elk naar zijn eigen basisbranch; mislukt er één, dan noemt de reden de repository. Een referentie-repository staat ondiep uitgecheckt op zijn eigen branch met pushen uitgeschakeld: de agent leest erin en verandert er niets aan. Changes en code review tonen de bestanden van alle aangepaste repositories, met de mapnaam ervoor. Runners moeten voor Jobs met meer repositories minimaal v1.28.62 zijn.
...
**Als een beurt misgaat.** Antwoordt de agent met een fout (een verlopen login, een provider die weigert) of eindigt zijn beurt zonder dat hij iets heeft gezegd, dan blijft de stap niet stil op "bezig" staan: Spin zet hem terug in de wachtrij met de reden op de kaart, sluit die agentsessie zodat de volgende poging met een verse begint, en probeert het opnieuw. Draait er een pool met meer logins, dan kan die volgende poging een ander account treffen.

**Eerst brainstormen.** Schieten op een goal is de norm. Wil je eerst praten, dan vul je alles in, met de goal als concept-goal, en kies je **Eerst brainstormen** in plaats van **Job inschieten**. De Job begint dan met een chat in zijn eigen omgeving (de agent heeft de repository voor zich en kan kijken wat er al is) over die concept-goal, met één vaste opdracht: dit is een brainstorm, geen uitvoering; verken, denk mee, leg mogelijkheden naast elkaar, werk toe naar één scherpe goal. De agent begint niet uit zichzelf: hij wacht op jouw eerste bericht en krijgt zijn instructies daarmee mee, zodat hij niet van alles gaat uitzoeken voordat je iets hebt kunnen vragen. De agent heeft in die chat precies één workflowtool, `start_process(goal)`, en roept die aan zodra jullie het eens zijn. Daarmee staat de goal op de Job en start stap 1 van de Template, zoals bij elke Job. De brainstorm staat op de kaart als eerste run, met zijn chat, zodat je later terugleest hoe de goal ontstond. Meer beslist de agent niet: repository, omgeving, Template en routes stonden al vast.

**Afronden.** KLAAR is klaar: een route naar **Klaar · Job afronden** beëindigt de Job, niets meer. Hoe het werk op de remote komt, is een stap in het Template zoals elke andere, met de soort **Mergen** (Spin merget de Job-branch in de basisbranch, altijd als merge-commit met de Job als onderwerp, en pusht met jouw credentials) of **Pull request** (Spin maakt een pull request op de remote; jij merget daar). Zo'n stap heeft gewone ACCEPT- en REJECT-routes; een mislukte merge of pull request is een gewone REJECT, en wie de route naar "vraag de gebruiker" laat lopen, kan de stap opnieuw laten proberen. Een persoon kiest nooit iets anders dan ACCEPT of REJECT; wat dat betekent bepaalt het Template, ook als de agent zelf accepteert.

**Een conflict door de agent laten oplossen.** Wie het conflict door een agent wil laten oplossen, laat de REJECT-route van de Merge-stap naar een agent-stap "AI merge" wijzen met als opdracht: merge `origin/<basisbranch>` in de Job-branch, los de conflicten op en commit. De basisbranch staat bij de start van elke stap al opgehaald in de workspace, dus de agent hoeft niets te fetchen en krijgt geen credentials; hij werkt alleen op de Job-branch, en Spin publiceert die zoals na elke beurt. De ACCEPT-route van die agent-stap wijst terug naar de Merge-stap, die nu schoon merget en met jouw credentials in de basisbranch pusht. Het conflict zelf is een gewone REJECT met als reden welke bestanden botsen; die reden komt als feedback in de prompt van de volgende stap, zoals elke afwijzing. Neemt een stap de basisbranch in zijn eigen branch op, dan houdt Spin die merge bij het accepteren vast: de resultaatcommit op de Job-branch krijgt de basisbranch als tweede ouder. Zonder dat wist de Job-branch niet dat de basis al binnen was en liep de volgende Merge-stap tegen precies dezelfde conflicten aan. Dat oplossen is een eigen **Soort** stap: **AI merge**. Een gewone agentstap blijft een gewone agentstap; alleen bij deze soort merget Spin de basisbranch in de Session-branch terwijl het de workspace klaarzet, in de container waar de Git-credentials zijn, zodat de agent de merge al begonnen aantreft met de conflictmarkers in de bestanden en MERGE_HEAD gezet. Zo'n stap mag altijd schrijven en krijgt in zijn prompt de werkwijze, boven op jouw eigen instructie: merge `origin/<basisbranch>` in deze workspace, los elk conflictbestand op, `git add` en `git commit` (de enige plek waar de agent zelf commit), en bouw de merge nooit met de hand na, want de volgende Merge-stap heeft een echte merge-commit nodig. De workspace blijft ondiep, maar beide branches worden bij het klaarzetten net zo ver verdiept tot ze de commit delen waar de Job vandaan komt; zonder die commit weigert git met "unrelated histories", en de agent kan zelf niets ophalen omdat hij geen Git-credentials heeft. Een merge die op een conflict strandt volgt meteen zelf de REJECT-route naar de oplosstap, met het conflict als reden; pas als de teller van het Template op is komt het bij jou, met OPNIEUW (voer de actie opnieuw uit) en REJECT (volg de route alsnog). Een merge die helemaal niet kán draaien (geen runner, een runner die te oud is, een verkeerde instelling) gaat niet die route op, want daar lost niemand iets op: die stopt direct bij jou met wat eraan mankeert.

**Een lopende Job op een nieuwe Template-revisie.** Een Job draait op een kopie van zijn Template, zodat een wijziging aan het Template een lopende Job niet onder zijn voeten verandert. Staat het Template op een nieuwere revisie, dan toont de Job-kaart de knop **Template rN**: die neemt de revisie over en vraagt bij welke stap van de nieuwe revisie de Job verdergaat (de stap met dezelfde naam staat voorgeselecteerd). Dat sluit de huidige stap af zonder oordeel ("Overgezet naar stap X"), laat een open vraag vervallen en start de gekozen stap als nieuwe poging; de branch, deliverables en geschiedenis blijven. Zo zet je een Job die vastliep op een Merge-stap om naar een revisie met een AI-merge-stap.
...
**Minimale rechten.** Spin doet met Git precies drie dingen: branches lezen, branches pushen (Session-branch, Job-branch, basisbranch bij een Merge-stap) en een pull request aanmaken. Meer rechten heeft het niet nodig. Voor GitHub is de scherpste vorm een fine-grained token of GitHub App op alleen de gekozen repositories met **Contents: Read and write**, **Pull requests: Read and write** en Metadata (read); voor GitLab een project access token met `read_repository` en `write_repository`. De OAuth-koppeling van GitHub kan niet fijner dan de klassieke `repo`-scope: wil je minder, gebruik dan een token-account in plaats van de OAuth-knop. Wil je dat Spin de basisbranch helemaal niet aanraakt, zet dan branch protection op `main`/`develop` en sluit de Template af met een **Pull request**-stap in plaats van **Mergen**: Spin pusht dan alleen nog `jobs/…`-branches en maakt de PR, en een mens klikt de merge. De werkkopie op de runner is van Spin alleen: de agent zit in een capsule zonder Git-credentials en kan er niet bij; klonen, syncen, accepteren en mergen gebeuren buiten die capsule. Voor echte provider-login open je als admin Connections → Git. Spin toont voor GitHub en GitLab de exacte callback-URL en een link naar de provider waar je de OAuth application maakt. Plak daarna Client ID en Client secret in Spin; het secret wordt versleuteld opgeslagen. `Koppel GitHub/GitLab` verschijnt zodra de provider klaarstaat.
