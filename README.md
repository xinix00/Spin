...
Een fase kan ook een eigen laag noemen. De lagen liggen dan op volgorde: eerst die van de Job, daarbovenop die van de fase. Noemt de fase een laag met een agent (een review door een ander model), dan draait die fase als die agent, met de credential-laag van de werker voor die agent erbij; de laag van de Job blijft eronder liggen. Noemt ze een laag zonder agent (een extra toolset), dan houdt de fase de agent van de Job. De laag die bovenop ligt bepaalt welk agentcommando start. Is een laag na een EDIT nieuwer dan de versie waarop een andere laag in de stapel is opgenomen (een credential op een oudere tool), dan legt de runner die nieuwe versie als eigen diff over de stapel: een EDIT bereikt zo iedere laag die erop gebouwd is.
