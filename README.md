...
**Afronden.** Een stap die de Job afrondt zegt in zijn ACCEPT-route hoe: **Job afronden · pull request maken** of **Job afronden · direct mergen in de basisbranch**. Dat is nodig omdat een agent die zelf mag accepten niemand kan vragen; de flow weet dan wat te doen. Vraagt de route wél om een persoon, dan kiest die bij de laatste ACCEPT alsnog: **ACCEPT · Pull request** of **ACCEPT · Mergen in <basisbranch>**, en die keuze wint voor die Job. Het Template geeft de standaard voor een route zonder keuze: als **pull request** op de remote (standaard; jij merget daar), of als **merge** door Spin zelf. Review en acceptatie zijn in Spin al gebeurd, dus met Mergen is er geen pull request meer nodig: de laatste fase brengt een workspace op met de omgeving van de Job en merget daaruit de Job-branch in de basisbranch (fast-forward als dat kan, anders een merge-commit met de Job als onderwerp), pusht en controleert de remote. Een merge die niet schoon lukt, wordt een beslissing voor een persoon, net als een mislukte pull request.
