...
Wat een laag geschreven heeft, is van die laag, en elk pad heeft een soort: tool (`/usr`, `/opt`), inlog (`~/.claude`, `~/.codex`, `~/.gemini`, `~/.config/gh`), home, config (`/etc/spin`), workspace, data, of cache (`~/.cache`, `~/.npm`, `/tmp`, `/var/cache`). Bij End & save houdt een laag alleen zijn echte verschil over: Docker rekent een bestand als gewijzigd zodra het voor schrijven is geopend, dus een tool die zichzelf ongewijzigd terugschreef of een login die een hele installatie aanraakte, maakte een laag van honderden megabytes die niets toevoegde. Spin leest de diff van de laag, vergelijkt elk bestand met de laag eronder, laat weg wat byte voor byte gelijk is en wat cache is, en bouwt de laag opnieuw uit de rest als er iets wegviel. De laagkaart toont de inhoudsopgave: aantal bestanden, grootte per soort met de grootste paden, en wat is weggelaten; de knop Inhoud opent de volledige lijst, grootste eerst, met filter op pad en soort, zodat je ziet wat er mis ging als een laag niet bevat wat je bedoelde. Die lijst staat als bestand naast de state, niet erin. Na elke beurt en bij het stoppen noteert Spin ook wat een capsule buiten de workspace veranderde, per soort, op de compositiekaart, met Bekijk voor de lijst; de inlogsoort daarvan is de inlogstaat hieronder.

Een agent ververst zijn OAuth-token terwijl hij werkt, en zo'n refresh-token is eenmalig. Bleef dat in de capsule achter, dan startte de volgende Session met het oude token uit de laag en weigerde de provider ("another process is refreshing it"). Spin bewaart daarom de inlogstaat: de bestanden die de opname van een credential-laag onder HOME schreef (het inloggen zelf) leest hij na elke beurt en bij het stoppen uit de capsule terug, versleuteld per laag en gebruiker in de database, en zet hij vóór de volgende agentstart weer neer. Welke bestanden dat zijn, volgt uit de diff van de laag, dus dat werkt voor elke agent zonder lijst; alles wat de agent verder in de capsule deed, verdwijnt met de capsule.
