...
Een Job krijgt `jobs/<naam-id>/main` als remote bron van waarheid; met een referentie (ticketnummer) wordt dat `jobs/<referentie>/<naam-id>/main`, zodat alle Jobs op hetzelfde ticket bij elkaar staan. Een fork erft de referentie van zijn bron. Vóór de eerste agent start maakt de kortlevende Git-helper die ref vanaf de gekozen basisbranch en leest hem opnieuw terug; iedere Session begint daarna vanaf die remote Job-head. Iedere Session krijgt een echte lokale `jobs/<naam-id>/sessions/<session-id>` in haar geïsoleerde workspace en wijst terug naar de Job-branch als reviewdoel. De namen zijn siblings omdat Git geen branch en een childref onder exact diezelfde branchnaam toestaat. Vanuit dezelfde Job-kaart kan een mens een extra Session starten en met `spawned_by_session_id` kan een Session/agent dat via de API ook zelf doen. Review/promotie naar de Job-branch blijft een orchestratoractie; agents ontvangen geen remote credential.
...
- `add_deliverable(name, content)` bewaart een benoemde Markdown-bijlage als nieuwe revisie en overschrijft daarmee het hele document; `edit_deliverable(name, old_text, new_text, all)` vervangt alleen een letterlijk stuk tekst in de laatste revisie (old_text moet precies één keer voorkomen, tenzij `all`) en `read_deliverable(name)` geeft de huidige tekst terug. Deze drie bestaan alleen in een fase die deliverables vraagt. Eén fase-poging levert één revisie: zolang dezelfde poging schrijft of bewerkt, wordt haar revisie bijgewerkt; een revisie met comments blijft staan en een volgende poging begint een nieuwe. Iedere revisie is in de browser te downloaden als `<naam>-r<revisie>.md`;
