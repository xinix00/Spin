...
Wat een laag geschreven heeft, is van die laag, en elk pad heeft een soort: tool (`/usr`, `/opt`), inlog (`~/.claude`, `~/.codex`, `~/.gemini`, `~/.config/gh`), home, config (`/etc/spin`), workspace, data, of cache (`~/.cache`, `~/.npm`, `/tmp`, `/var/cache`). Bij End & save houdt een laag alleen zijn echte verschil over: Docker rekent een bestand als gewijzigd zodra het voor schrijven is geopend, dus een tool die zichzelf ongewijzigd terugschreef of een login die een hele installatie aanraakte, maakte een laag van honderden megabytes die niets toevoegde. Spin leest de diff van de laag, vergelijkt elk bestand met de laag eronder, laat weg wat byte voor byte gelijk is en wat cache is, en bouwt de laag opnieuw uit de rest als er iets wegviel. De laagkaart toont de inhoudsopgave: aantal bestanden, grootte per soort met de grootste paden, en wat is weggelaten; de knop Inhoud opent de volledige lijst, grootste eerst, met filter op pad en soort, zodat je ziet wat er mis ging als een laag niet bevat wat je bedoelde. Die lijst staat als bestand naast de state, niet erin. Een laag van vóór de inhoudsopgaven krijgt de zijne alsnog: met Inhoud bepalen op de kaart, en op de achtergrond voor elke laag die er nog geen heeft zodra een runner opkomt; een runner leest dan het verschil uit de bovenste Docker-laag van de image, zonder nieuwe opname. Na elke beurt en bij het stoppen noteert Spin ook wat een capsule buiten de workspace veranderde, per soort, op de compositiekaart, met Bekijk voor de lijst; de inlogsoort daarvan is de inlogstaat hieronder.
