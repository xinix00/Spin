...
Eén server draait meerdere Spins, één per domein. Het `Host`-header kiest de Spin: `bollenloods.getspin.app` krijgt `bollenloods.getspin.app.db` in de datamap (`SPIN_DATA_DIR`, standaard `/data` op HopOS en `./var` elders), met een eigen store, eigen runners, eigen worker-token en eigen replica. De server neemt elk domein aan dat binnenkomt; wat ervoor routeert bepaalt wat er binnenkomt, er hoeft niets vooraf te worden vastgelegd. Een IP-adres als host is nooit een Spin en beantwoordt alleen `/healthz`. Bij het opstarten opent de server elke Spin die hij al heeft (de databases in de datamap, en de domeinen met een replica in de bucket, die zo nodig eerst worden hersteld), zodat hun replica's vanaf de eerste minuut lopen. `SPIN_DOMAINS` (kommagescheiden) kan de lijst optioneel beperken. `SPIN_DATABASE` zet de server terug in de oude vorm: één database voor elke host. `SPIN_PUBLIC_URL` is per domein `https://<domein>` tenzij anders gezet; `SPIN_MASTER_KEY` geldt voor alle databases op de server.

### Replica in S3

Een Spin zonder replica is geen Spin: de server start niet zonder `SPIN_S3_ENDPOINT`, `SPIN_S3_BUCKET`, `SPIN_S3_ACCESS_KEY` en `SPIN_S3_SECRET_KEY` (optioneel `SPIN_S3_REGION`, standaard `auto`, en `SPIN_S3_PREFIX`, standaard `spin`). Alleen een ontwikkelserver zet `SPIN_REPLICATION=off`. De replica werkt op paginaniveau, in de geest van Litestream maar zonder WAL en zonder bestandssysteem, want die heeft HopOS niet: een VFS-laag tussen SQLite en de opslag onthoudt welke pagina's zijn geschreven, en iedere vijftien seconden leest de server die pagina's onder een leestransactie (dus nooit halverwege een write) en stuurt ze als segment naar `<prefix>/<domein>/generations/<id>/<volgnummer>.seg`. Een generatie begint met een snapshot van alle pagina's en groeit met de wijzigingen; `current` wijst naar de generatie die compleet is. Wegen de wijzigingen twee keer de database, of is de generatie een week oud, dan begint een nieuwe generatie; generaties blijven vier weken staan.

Herstelpunten dunnen uit met de leeftijd, zoals Litestream compacteert. De ruwe segmenten (één per sync) worden na afloop van een kwartier samengevoegd tot één kwartiervenster met van elke gewijzigde pagina de laatste staat, kwartieren tot uren, uren tot dagen; de fijnere bestanden verdwijnen zodra een grover venster ze dekt en hun bewaartermijn om is. Standaard: kwartieren voor twee uur, uren voor een dag, dagen voor een week, en de wekelijkse generaties voor een maand (`SPIN_REPLICA_SCHEDULE=15m:2h,1h:24h,24h:168h`, `SPIN_REPLICA_GENERATION=168h`, `SPIN_REPLICA_RETENTION=672h`). Samenvoegen gebeurt in twee doorgangen (eerst welk bestand de laatste staat van elke pagina heeft, dan die pagina's overnemen), dus het geheugen blijft bij één invoerbestand en één uitvoerdeel van 16 MiB, hoe groot het venster ook is. Terugzetten naar een moment legt de snapshot neer, dan de grofste vensters die vóór dat moment eindigen, dan fijnere voor de rest, dan de ruwe segmenten tot het moment. Onder Backup & restore toont een admin de herstelpunten, nieuwste eerst, en zet er een terug; dat loopt door dezelfde gevalideerde restore als een backup-zip, alleen zonder upload, en de staat van vóór het herstel blijft zelf een herstelpunt. Bij het opstarten wordt een ontbrekende database uit de bucket herbouwd; een database die er wel is, gaat verder in zijn generatie als het merkteken naast de database zegt dat alles verstuurd was, en begint anders een nieuwe. Segmenten dragen een checksum en de S3-client tekent met SigV4 zonder SDK. De opslagregel onder Connections → Runners en `/healthz` melden bucket, generatie, laatste sync, wachtende pagina's en fouten. De handmatige backup-zip en restore blijven bestaan; een restore loopt door dezelfde VFS en komt dus vanzelf in de replica.

De HopOS-server verwacht een gepubliceerde `ER_PORT_HTTP`, een gemounte `/data` voor de databases, de S3-instellingen en een vaste base64 `SPIN_MASTER_KEY`. SQLite v0.35.4 is libc-vrij via wasm2go; een eigen HopOS-VFS vertaalt random-access pagina-I/O naar de volume-ABI. De echte releasegate bouwt dit pad voor arm64 én riscv64. Control plane, runner-WebSocket, Job-bijlagen, centrale Docker-snapshots, databasebackup en replica zijn daardoor op beide targets beschikbaar.
