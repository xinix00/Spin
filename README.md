...
Een Job krijgt `jobs/<naam-id>/main` als remote bron van waarheid; met een referentie (ticketnummer, bijvoorbeeld `#1234`) wordt dat `jobs/<referentie>/<naam-id>/main`, zodat alle Jobs op hetzelfde ticket bij elkaar staan. Een fork erft de referentie van zijn bron. Vóór de eerste agent start maakt de kortlevende Git-helper die ref vanaf de gekozen basisbranch en leest hem opnieuw terug; iedere Session begint daarna vanaf die remote Job-head. Iedere Session krijgt een echte lokale `jobs/<naam-id>/sessions/<session-id>` in haar geïsoleerde workspace en wijst terug naar de Job-branch als reviewdoel. De namen zijn siblings omdat Git geen branch en een childref onder exact diezelfde branchnaam toestaat. Vanuit dezelfde Job-kaart kan een mens een extra Session starten en met `spawned_by_session_id` kan een Session/agent dat via de API ook zelf doen. Review/promotie naar de Job-branch blijft een orchestratoractie; agents ontvangen geen remote credential.
...
Per fase kies je optioneel een **model** en een **reasoning-niveau**. Spin zet die vóór de eerste prompt op de ACP-sessie van de agent (`session/set_config_option` met `model` en `reasoning_effort`; een agent die zijn modellen als `models` bij `session/new` meldt, zoals Claude Code, krijgt het model via `session/set_model` en een reasoning-niveau alleen als hij er een aanbiedt), zodat een routinestap een klein model kan draaien en een ontwerpstap een groot, zonder aparte lagen per model. Welke waarden een agent aanbiedt, staat op zijn laag: **Opties ophalen** op een ACP-laag onder Environments start de agent één keer in een wegwerp-capsule, leest de `configOptions` (of `models` en `modes`) van `session/new` en bewaart ze; iedere gestarte Session ververst ze gratis. De Template-editor biedt die waarden aan. Een waarde die de agent weigert, houdt de fase in de wachtrij met de reden op de kaart.
