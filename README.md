...
**Visuele deliverables.** Bij een deliverable in het Template kies je **Document** (Markdown) of **Visueel**. Een visuele revisie wordt door de runner als zip uit de capsule gehaald (map of bestand, ten hoogste 25 MB) en in de database bewaard; de reviewer ziet hem in de viewer in een iframe en kan hem in een eigen tab openen, met revisies naast elkaar en commentaar op de hele revisie. Elke geserveerde pagina krijgt een Content-Security-Policy met `sandbox allow-scripts`, `connect-src 'none'` en `form-action 'none'`: een eigen lege origin, geen toegang tot Spin, geen netwerk, wel werkende scripts en formulieren binnen de preview. Zo kan een ontwerpstap een printformulier of een shop-styling laten zien met de echte CSS en JS, en een ontwikkelstap een screenshot of PDF.
