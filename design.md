...
De probe start de entrypoint in de gematerialiseerde snapshot, onderhandelt ACP v1 en sluit daarna het subprocess. Dit verifieert dus de echte laag, niet alleen de metadata. De normale GUI-flow gebruikt `GET /api/sessions/{id}/acp`: een WebSocket abonneert zich op een server-side supervisor die één ACP-subprocess per Spin Session in leven houdt. Het sluiten van het scherm verbreekt alleen de browserverbinding; de agent en een lopende beurt blijven a[text](https://lgflowers-test.easyflor.eu/)ctief. Heropenen speelt de vluchtige events van die serverruntime opnieuw af.
...
- een fase zonder `allow_changes` integreert nooit: ACCEPT bevestigt de Session-base en laat wijzigingen in de workspace achter;
